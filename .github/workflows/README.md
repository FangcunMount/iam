...
- `status`: 只读输出 MySQL 客户端版本、连接状态、库总大小、表数量、schema 对象、备份摘要、`schema_migrations` 和迁移锁；当前 fail closed 验证 `version=27, dirty=0`、16 张现役 BASE TABLE 精确白名单，以及历史退役表和 `casbin_rule` 持续不存在。RoleBinding 结构与重复事实由独立 preflight 持续检查。
