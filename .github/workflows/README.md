...
- `status`: 只读输出 MySQL 客户端版本、连接状态、库总大小、表数量、schema 对象、备份摘要、`schema_migrations` 和迁移锁；当前仓库脚本面向 000028 发布后目标，fail closed 验证 `version=28, dirty=0`、16 张现役 BASE TABLE 精确白名单，以及历史退役表和 `casbin_rule` 持续不存在。最近一次生产证据仍为 27，发布前运行新版 status 会按预期失败。Assignment 结构与重复事实由历史命名的独立 preflight 持续检查。
