...
自 `000021_retire_unused_platform_tables` 起，bootstrap 不再维护 `tenants` 主数据或 `data_dictionary` 字典；AuthZ 使用的 `platform`、`fangcun` tenant domain 继续直接保存在角色、分配、继承、PermissionGrant 和策略版本中，不依赖已退役的平台辅助表。
