...
这个设计允许 password、phone_otp、wechat、wecom 等登录方式通过 `LoginMethod`、`ProofFactory` 和领域 strategy 扩展，而不是在 handler 或 login service 里堆巨大条件分支。bearer access token 再验证走独立的 `Reauthenticate` / `TokenVerifier` 链路。

```mermaid
flowchart TD
    Request["Login Request"]
    Registry["MethodRegistry / LoginMethod"]
    ...
    Strategy --> SessionIssuer
    SessionIssuer --> Session
    SessionIssuer --> PairIssuer
    PairIssuer --> JWT
    PairIssuer --> Refresh
    JWT --> JWKS
```
...
当前用户视角通过 `MyProfileLinks` 做边界保护，防止用户操作其他用户的关系。self profile/link 只在用户主动为自己创建档案时建立，active self 唯一性由 `SelfProfileGuard` 和 `self_key` 唯一索引兜底。

核心源码：

- [internal/apiserver/domain/identity/profilelink](../../internal/apiserver/domain/identity/profilelink)
- [internal/apiserver/application/identity/profilelink](../../internal/apiserver/application/identity/profilelink)
- [internal/apiserver/application/identity/uow](../../internal/apiserver/application/identity/uow)
- [internal/apiserver/transport/rest/identity](../../internal/apiserver/transport/rest/identity)
- [internal/apiserver/transport/grpc/service/identity](../../internal/apiserver/transport/grpc/service/identity)
