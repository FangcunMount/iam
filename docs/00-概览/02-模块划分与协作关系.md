...
| 能访问什么 | AuthZ | Subject、Resource、Action、Scope、Role、Permission、RoleBinding |
| 外部身份来源如何接入 | IDP | WechatApp、Credentials、AppToken、ExternalIdentity |
| 如何快速搜索可见 Profile | Suggest | ProfileSearchTerm、ProfileAccessScope、ProfileSuggestionIndex |
...
```text
维护资源、动作、范围、角色、权限和角色绑定；
执行权限检查；
管理授权写入；
通过 PolicyVersion 和 Outbox 传播授权事实变化；
使用 Casbin 等 infra runtime 完成运行时匹配。
```
...
sequenceDiagram
    ...
    Resource->>AuthZ: Check(subject, resource, action, scope)
    AuthZ->>Identity: 按需引用 User/Profile 事实
    Identity-->>AuthZ: Identity fact
...
sequenceDiagram
    ...
    Suggest->>Identity: 消费 Profile 事实或索引快照
    Identity-->>Suggest: Profile facts
    Suggest->>AuthZ: 获取或使用 ProfileAccessScope
    AuthZ-->>Suggest: 可见范围
    Suggest->>Suggest: 索引召回 + 权限过滤 + 脱敏
    Suggest-->>Client: ProfileSuggestItem[]
```
...
| ProfileLink 连接 User 和 Profile，所以它是权限 | ProfileLink 是身份关系事实，不是 Permission |
| Suggest 搜索 Profile，所以 Suggest 拥有 Profile | Profile 属于 Identity，Suggest 只消费读事实和索引快照 |
| Suggest 做可见范围过滤，所以 Suggest 是授权系统 | Suggest 使用 ProfileAccessScope，不管理通用授权策略 |
| Casbin 做权限匹配，所以 Casbin 是业务模型 | Casbin 是 infra runtime，业务模型仍是 AuthZ 领域对象 |
...
| AuthZ | Subject、Resource、Action、Scope、Role、Permission、RoleBinding、AuthorizationDecision、PolicyVersion | Identity.User/Profile 引用；AuthN 认证结果 | 登录认证、Token 签发、User/Profile 写模型 |
| IDP | WechatApp、Credentials、AppToken、ExternalIdentity | 外部身份源 API | IAM 登录态、IAM Token、User 所有权、权限判定 |
| Suggest | ProfileSearchTerm、ProfileAccessScope、ProfileSuggestionIndex、ProfileSuggestItem | Identity.Profile；AuthZ 可见范围 | Profile 写模型、登录认证、通用授权策略管理 |
...
想理解 REST/gRPC/SDK 接入：读 03-接入与契约/README.md。
想理解为什么这样设计：读 05-专题设计/README.md。
想准备面试讲法：读 06-宣讲/README.md。
```
