...
这张图表达的是模块事实输入和窄能力协作，不是代码包导入关系。图中的箭头必须连同标签阅读：例如 AuthN 使用 `UserStatusReader`、AuthZ 使用 `UserResolver`、Identity 使用
`SessionRevoker` 和 `RoleNameReader`、Suggest 使用 `RouteAuthorization`；这些端口由组合根装配，不等于模块共享聚合。
