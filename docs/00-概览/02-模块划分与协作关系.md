...
![IAM 模块边界与窄能力协作关系](../_images/architecture/module-boundary.png)

[SVG 图源](../_images/architecture/module-boundary.svg)

这张图表达的是模块事实输入和窄能力协作，不是代码包导入关系。图中的箭头必须连同标签阅读：例如 AuthN 使用 `UserStatusReader`、AuthZ 使用 `UserResolver`、Identity 使用 `SessionRevoker` 和 `RoleNameReader`、Suggest 使用 `RouteAuthorization`；这些端口由组合根装配，不等于模块共享聚合。
...
从 MySQL 事实编译原生不可变快照；Casbin role manager 装载 Assignment 与 RoleInheritance，只计算租户隔离的角色闭包。
```
...
| Casbin 做权限匹配，所以 Casbin 是业务模型 | 权限匹配由 IAM 原生 runtime 执行；Casbin 只在内存中计算角色继承 |
