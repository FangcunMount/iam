...
图里没有 `application -> infra` 的运行时依赖箭头。具体适配器由 `container` 创建并注入 Domain/Application 端口；请求执行期间，application 只持有端口或窄能力。AuthZ 的 Casbin 也被限定在 native snapshot 内部，只作为 RoleManager。
...
按 Resource/Action 索引 PermissionGrant；
校验 ObjectAttribute Schema 并求值 ConstraintSet；
返回 Decision、命中事实和实际加载版本；
在 version event 后完整 reload 并原子替换。
```

Casbin role manager 只负责：

```text
在快照内装载 Assignment 的 Subject→Role 边和 RoleInheritance 的 Role→Role 边，计算租户隔离的有效 Role 闭包。
```

Casbin role manager 不负责：
