...
图里没有 `application -> infra` 的运行时依赖箭头。具体适配器由 `container` 创建并注入 Domain/Application 端口；请求执行期间，application 只持有端口或窄能力。AuthZ 的
Casbin 也被限定在 native snapshot 内部，只作为 RoleManager。
