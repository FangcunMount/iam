...
AuthZ 的自有角色图被限定在 immutable snapshot 内部，只作为可重建索引。
...
领域 `AuthorizationEvaluator` 负责匹配 Resource/Action、校验 ObjectAttribute Schema、求值 ConstraintSet 并产生 Decision；runtime 索引本身不拥有这些判定规则。
