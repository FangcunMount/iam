...
flowchart TD
    ...
    Domain["domain\n领域模型 / 业务规则 / 领域端口"]
    Infra["infra\nMySQL / Redis / AuthZ Runtime / JWT / WeChat / Suggest Runtime"]
...
装配 token、AuthZ 原生 runtime、idp、suggest runtime 等技术能力；
把模块依赖集中收敛到一个可审计位置。
```
...
SQL 实现；
Redis 实现；
AuthZ 快照或 Casbin role manager 构造；
JWT 具体签名实现；
微信 HTTP API 调用细节。
```
...
```text
JWT adapter 负责签名和验签；
AuthZ native runtime adapter 负责构建不可变授权快照并完成 Grant 条件求值；
...
domain -> mysql/redis/native-authz/jwt/wechat sdk；
...
> AuthZ 领域模型先成立，再编译为原生不可变快照；Casbin 只实现快照中的角色图，不能反过来用 Casbin facts 定义业务模型。
...
| AuthZ 直接以 Casbin facts 作为领域语言 | 业务授权模型被工具吞并 | 以 Assignment/RoleInheritance/PermissionGrant 为事实，Casbin 只计算内存角色图 |
...
infra 负责数据库、Redis、AuthZ 原生快照与内存角色图、JWT、微信 API、Suggest Runtime 等技术实现。
```
