...
- Ports & Adapters、组合根、事务边界、Outbox、Casbin、JWT/JWKS、Suggest Runtime 在架构中分别处于什么位置？
...
flowchart TD
    ...
    Rest["transport/rest\nHTTP 路由 / DTO / middleware"]
    Grpc["transport/grpc\ngRPC service / interceptor"]

    Application["application\n用例编排 / 事务边界 / 端口调用"]
    Domain["domain\n领域模型 / 业务规则 / 领域端口"]
    Infra["infra\nMySQL / Redis / Casbin / JWT / WeChat / Suggest Runtime"]
...
装配 token、casbin、idp、suggest runtime 等技术能力；
把模块依赖集中收敛到一个可审计位置。
```
...
定义 Subject / Resource / Action / Scope / Role / Permission / RoleBinding；
定义 WechatApp / ExternalIdentity 等 IDP 领域对象；
定义 ProfileSearchTerm / ProfileAccessScope / ProfileSuggestionIndex 等 Suggest 领域对象；
表达领域不变量；
定义领域端口接口。
```
...
flowchart LR
    ...
    Infra --> External["Casbin / JWT / WeChat / Suggest Runtime"]
```
...
| AuthN | LoginIdentity、Credential、Challenge、Principal、Session、Token、JWKS | Role、Permission、ProfileLink、Casbin 策略 |
| AuthZ | Subject、Resource、Action、Scope、Role、Permission、RoleBinding、Decision、PolicyVersion | 登录认证、Token 签发、User/Profile 写模型 |
...
## 11. AuthZ 与 Casbin 原则

Casbin 在 IAM 中应被理解为 infra runtime，而不是 AuthZ 领域模型本身。
...
> AuthZ 领域模型先成立，再投影到 Casbin runtime；不能反过来用 Casbin facts 定义业务模型。
...
Suggest 是 Profile 联想搜索读模型，其 runtime 可以使用进程内索引、快照、刷新任务等技术实现。
...
登录认证；
通用授权策略管理；
替代 AuthZ Permission / RoleBinding。
```
...
infra：封装外部依赖错误，例如数据库、Redis、微信 API、Casbin runtime。
```
...
架构护栏细节见 [04-架构护栏](../04-架构护栏/README.md)。
...
03-接入与契约：REST/gRPC/SDK 接入和契约事实源；
04-架构护栏：分层依赖、测试、防漂移机制；
05-专题设计：设计取舍、替代方案、为什么这样设计；
06-宣讲：面试和技术分享表达；
...
| AuthZ 直接以 Casbin facts 作为领域语言 | 业务授权模型被工具吞并 | 先定义 Role/Permission/RoleBinding，再投影到 Casbin |
...
| ProfileLink 当 Permission 用 | 身份关系和授权事实混淆 | ProfileLink 归 Identity，Permission 归 AuthZ |
...
infra 负责数据库、Redis、Casbin、JWT、微信 API、Suggest Runtime 等技术实现。
```
