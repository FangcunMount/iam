...
每次授权或搜索直接查规范化表实现简单一致，但高频路径会复制复杂 join/matcher，数据库成为延迟和可用性瓶颈。当前选择 AuthZ 不可变原生快照与 Suggest 派生运行时，将写一致性和读性能解耦；Casbin
只参与快照内的角色图计算。
...
| 在线身份检查 | `internal/apiserver/domain/authn/token`、AdmissionPolicy 与对应 adapters |
