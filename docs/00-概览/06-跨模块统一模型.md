...
| RoleInheritance | Role -> inherited Role | 角色继承哪些稳定能力 | AuthZ；运行时由内存 Casbin role manager 计算闭包 |
...
每次授权或搜索直接查规范化表实现简单一致，但高频路径会复制复杂 join/matcher，数据库成为延迟和可用性瓶颈。当前选择 AuthZ 不可变原生快照与 Suggest 派生运行时，将写一致性和读性能解耦；Casbin 只参与快照内的角色图计算。
...
| AuthZ 写入与运行时 | `internal/apiserver/application/authz`、`internal/apiserver/infra/authz/native` |
...
```bash
go test ./internal/apiserver/container/... ./internal/apiserver/application/... ./internal/apiserver/infra/authz/native/... ./internal/apiserver/infra/suggest/...
```
