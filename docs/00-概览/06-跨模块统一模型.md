...
- ProfileLink、RoleBinding、Permission 为什么不能合成一种“关系”？
- JWT 已经带有主体声明，为什么请求还可能需要 Session 检查和 AuthZ Check？
- MySQL 是事实源，为什么运行时还需要 Redis、Casbin 内存策略和 Suggest 索引？
...
| Role、Permission、RoleBinding、PolicyVersion、Casbin policy | AuthZ | MySQL | 授权配置变化 |
...
`ExternalIdentity` 不是 User，因为 provider 只对自己的标识负责；`Principal` 也不是 User 表，因为它描述“本次认证以什么方式、在哪个租户和会话中成为谁”。证明会过期、可撤销，也可能只适用于特定 audience。
...
| authorization | Subject、Tenant、Resource、Action、Scope、当前 policy | allow/deny + reason/version | PermissionDenied |
...
```text
decision = f(subject, tenant, resource, action, object_scope, policy_snapshot)
```
...
| 投影 | 来源 | 用途 | 失效方式 |
| --- | --- | --- | --- |
| Casbin CachedEnforcer | AuthZ MySQL policy | 低延迟 Check | version event 后 reload |
| Suggest Trie/Hash Store | Identity/ProfileLink/User 查询 | 联想召回与排序 | Full 原子替换、Delta 修正 |
...
flowchart LR
    ...
    SUB --> REQ["Tenant + Resource + Action + Scope"]
...
```text
Identity facts
  -> Suggest Loader 派生 ProfileSearchTerm
  -> Full/Delta 更新进程内索引
  -> AuthN Principal + AuthZ route permission
...
从图数据库角度看，ProfileLink、RoleBinding 和 Casbin `g` 都像“边”，但业务语义不同：

| 边 | 起点/终点 | 回答的问题 | 生命周期所有者 |
| --- | --- | --- | --- |
| ProfileLink | User -> Profile | 两者是什么业务关系 | Identity |
| RoleBinding | Subject -> Role@Scope | 某授权域绑定什么角色 | AuthZ |
| Permission | Role -> Action@Resource | 角色拥有什么动作能力 | AuthZ |
| Casbin g/p | runtime strings | 如何高效执行绑定和规则 | infra/casbin |

一个 guardian ProfileLink 可以是生成授权输入的依据，但不是 `read/update/export/delete` 四个动作的默认结论。若把它直接写成 Permission：

- 关系撤销与权限撤销难以独立审计；
- 新动作会被旧关系隐式放行；
- Identity repository 被迫理解 Casbin；
- Suggest 会把“有关系”误当成“可见”。

正确方式是显式策略：关系事实进入授权规则或 scope resolver，再由 AuthZ 对动作作决定。
...
- AuthZ 对敏感操作读取当前 runtime policy；
...
当前拆分 User/Profile/ProfileLink、LoginIdentity/Credential、RoleBinding/Permission，使每类事实按自己的生命周期演化。代价是需要明确跨模块用例和映射，不能再靠一条 SQL 得到所有结论。
...
每次授权或搜索直接查规范化表实现简单一致，但高频路径会复制复杂 join/matcher，数据库成为延迟和可用性瓶颈。当前选择 Casbin/Suggest 派生运行时，将写一致性和读性能解耦。
...
| 在线身份检查 | `internal/apiserver/application/authn/token`、subject access adapters |
| AuthZ 写入与运行时 | `internal/apiserver/application/authz`、`internal/apiserver/infra/casbin` |
| 事件语义 | `configs/events.yaml`、`internal/apiserver/infra/event` |
...
```bash
/Users/yangshujie/.gvm/gos/go1.25.12/bin/go test ./internal/apiserver/container/... ./internal/apiserver/application/... ./internal/apiserver/infra/casbin/... ./internal/apiserver/infra/suggest/...
```
