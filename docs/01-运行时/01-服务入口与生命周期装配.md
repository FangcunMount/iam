...
- HTTP/gRPC 如何被启动；
- key rotation、outbox relay、AuthZ policy sync、suggest cleanup 等后台任务如何纳入生命周期；
- graceful shutdown 的关闭顺序是什么；
- degraded startup 到底允许什么、不允许什么。

本文只讨论运行时装配，不展开 AuthN、AuthZ、ProfileLink、PolicyVersion、Outbox 的业务细节。业务链路分别在 `docs/02-认证AuthN`、`docs/03-授权AuthZ`、`docs/04-身份Identity`、`docs/05-接入与契约` 中单独讨论。
...
- `container` 是组合根，负责装配 AuthN、AuthZ、User、IDP、Suggest、CacheGovernance、Eventing/Outbox，并在 EventBus 可用时构建 AuthZ policy sync subscriber。
- `transport/rest` 和 `transport/grpc` 是协议适配层，只消费 container 投影出来的 deps/registration，不直接拥有模块初始化逻辑。
- `User Module` 是运行时代码中的历史/实现命名；在新版文档语义中，它归入 Identity 能力边界。
...
sequenceDiagram
    ...
    Process->>Process: start runtime tasks: key rotation / outbox relay / authz policy sync
...
| 后台任务在哪里启动 | `runtimeTaskStage` 调用 `startRuntimeTasks`，启动 key rotation、outbox relay、AuthZ policy sync。 | [../../internal/apiserver/process/shutdown_lifecycle.go](../../internal/apiserver/process/shutdown_lifecycle.go) |
| AuthZ policy sync 何时进入生命周期 | `Container.BuildRuntimeDeps()` 暴露 `AuthzPolicySync`，`startRuntimeTasks` 调用 `startAuthzPolicySync` 并注册 shutdown hook。 | [../../internal/apiserver/container/runtime_deps.go](../../internal/apiserver/container/runtime_deps.go)、[../../internal/apiserver/process/shutdown_lifecycle.go](../../internal/apiserver/process/shutdown_lifecycle.go) |
| shutdown 顺序在哪里定义 | lifecycle hooks -> suggest cleanup -> DB close -> HTTP close -> gRPC close；lifecycle hooks 中停止 outbox relay、key rotation scheduler、AuthZ policy sync subscriber。 | [../../internal/apiserver/process/shutdown_lifecycle.go](../../internal/apiserver/process/shutdown_lifecycle.go) |
...
| `start runtime tasks` | prepareState | lifecycle hooks | 启动 key rotation scheduler、outbox relay、AuthZ policy sync subscriber |
...
其中 `user module` 是当前运行时代码中的历史/实现命名；在新版文档语义中，它归入 Identity 能力边界，负责 User、Profile、ProfileLink 相关 capabilities。
...
| AuthZ | MySQL、outbox store；EventBus 可用时还会构建 AuthZ policy sync subscriber，用于订阅授权版本事件 |
...
| `AuthzPolicySync` | AuthZ module + EventBus subscriber | 启动授权版本事件订阅器；shutdown hook 中停止 |
...
flowchart TD
    ...
    Container --> Scheduler
    Container --> Relay
    Container --> PolicySync
    Container --> SuggestCleanup
    Scheduler --> Process
    Relay --> Process
    PolicySync --> Process
    SuggestCleanup --> Shutdown
    Process --> Lifecycle
```

### 10.3 AuthZ policy sync subscriber

如果 container 暴露了 `AuthzPolicySync`，process 会：

1. 调用 `startAuthzPolicySync(lifecycle, sync)`；
2. 执行 `sync.Start(context.Background())`；
3. 注册 shutdown hook：`stop authz policy sync subscriber`。

AuthZ policy sync subscriber 订阅的 topic 是：

```text
iam.authz.version
```

它和 Outbox Relay 的职责不同：

| 组件 | 职责 |
| --- | --- |
| Outbox Relay | 从 `domain_event_outbox` claim due events，并 publish 到 EventBus |
| AuthZ Policy Sync Subscriber | 订阅 `iam.authz.version`，消费授权版本事件并触发授权运行时同步 |
...
因此 EventBus 不可用时，Outbox Relay 不完整或不启动，AuthZ policy sync subscriber 也不会构建。
...
sequenceDiagram
    ...
    Life-->>GS: stop relay / stop scheduler / stop authz policy sync
...
| EventBus 创建失败 | 记录 warning，继续 | 记录 warning，继续 | EventBus 是可选外部消息资源；不可用时 Outbox Relay 不完整或不启动，AuthZ policy sync subscriber 不构建 |
...
- EventBus 不可用时，outbox relay 不完整或不启动，AuthZ policy sync subscriber 不构建，但 outbox store 仍可能保留 pending/failed 事件。
...
| 后台任务纳入生命周期 | scheduler/relay/policy sync 不能游离于 shutdown 外 | `BuildRuntimeDeps` + lifecycle hooks |
...
不准确。EventBus 不可用时，relay 不完整或不启动，AuthZ policy sync subscriber 也不会构建，事件保留在 outbox 中；业务事实和 outbox 记录仍可以在 MySQL 事务中提交。  
这取决于 MySQL/outbox store 是否可用。授权版本传播则需要等待 EventBus 恢复后的 relay / subscriber 链路，或依赖其他显式 reload 机制。
...
> `main` 只交出控制权，`app` 接入 options/config，`process` 编排生命周期，`container` 装配业务模块并暴露 runtime deps，`transport` 注册协议入口，`preparedAPIServer.Run` 运行 HTTP/gRPC 并接入 graceful shutdown。

这篇文档的重点不是“有哪些接口”，而是理解服务如何安全、可测试、可降级、可关闭地启动起来。后续阅读 AuthN、AuthZ、ProfileLink、PolicyVersion、Outbox 文档时，都应该先把这个生命周期模型放在脑子里。
