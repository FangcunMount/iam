...
- EventBus 配置如何影响 Outbox Relay 与 AuthZ Policy Sync Subscriber；
...
EventBus 是运行时事件链路的关键边界。NSQ/EventBus 不可用时，outbox store 仍可能保留 pending/failed 事件，但 Outbox Relay 不完整或不启动，AuthZ Policy Sync Subscriber 也不会构建；授权版本传播需要等待 EventBus 恢复后的 relay/subscriber 链路，或依赖其他显式 reload 机制。
...
| EventBus 何时创建 | `nsq.enabled=true` 时，process 尝试创建 EventBus；未启用或创建失败时 `eventBus=nil`。 | [../../internal/apiserver/process/event_bus.go](../../internal/apiserver/process/event_bus.go) |
| AuthZ Policy Sync 依赖什么 | AuthZ module 和 EventBus 都存在时，container 才会构建 `AuthzPolicySync`。 | [../../internal/apiserver/container/runtime_deps.go](../../internal/apiserver/container/runtime_deps.go) |
...
| 关键模块缺失 | 记录 warning |
| EventBus 创建失败 | 记录 warning，eventBus = nil；Outbox Relay 不完整或不启动，AuthZ Policy Sync Subscriber 不构建 |
...
| EventBus 缺失 | outbox relay 不完整或不启动，AuthZ Policy Sync Subscriber 不构建，授权版本传播链路不可完整运行 |
...
EventBus 为 nil 时，运行时影响包括：

```text
Outbox Relay 不完整或不启动；
AuthZ Policy Sync Subscriber 不构建；
domain_event_outbox 中的 pending/failed 事件仍可保留在数据库中；
授权版本事件不会通过 iam.authz.version topic 被本进程消费。
```
...
AuthZ Policy Sync Subscriber 不直接由 `events.outbox_relay_interval` 驱动。它依赖 AuthZ module 与 EventBus subscriber 构建，订阅授权版本 topic：

```text
iam.authz.version
```

因此，Outbox Relay 与 AuthZ Policy Sync 的边界是：Relay 负责把 outbox due events 发布到 EventBus；Policy Sync 负责消费授权版本事件并触发运行时授权状态同步。
...
- NSQ 默认关闭时，EventBus 为 nil，outbox relay 不会完整投递消息，AuthZ policy sync subscriber 也不会启动。
...
seed_mock_auth.enabled = false
  nsq.enabled 根据是否需要事件投递和 AuthZ policy sync 开启
...
- EventBus 不可用不等于业务写入失败，但会影响 outbox 事件投递与 AuthZ policy sync subscriber。
...
不一定。NSQ/EventBus 关闭主要影响事件投递和运行时事件消费。只要 MySQL/outbox store 可用，业务事实和 outbox 记录仍可提交；但 relay 不会完整发布消息，AuthZ policy sync subscriber 也不会构建，授权版本事件不会通过 `iam.authz.version` topic 被本进程消费。
...
EventBus 不可用会影响 Outbox Relay 与 AuthZ Policy Sync
```
