...
配置来源优先级必须以代码实现为准。文档中不应凭空定义覆盖规则。

如果代码已经实现优先级，应在本文后续版本补充成：

```text
默认值 < 配置文件 < 环境变量 < 启动参数
```

如果代码尚未明确优先级，应标记为 `待补证据`，不要写成已实现事实。
...
```text
运行模式：local/dev/test/staging/prod 等环境语义；
降级启动：依赖不可用时是否允许服务以 degraded 状态启动。
```
...
```text
local 模式可以使用 mock IDP；
prod 模式通常不允许使用 mock IDP；
Suggest 不可用可能允许 degraded；
主数据库不可用通常不应 ready；
JWT private key 不可用通常应 fail fast。
```
