...
> 状态：当前实现 · 已按配置结构、样例配置、启动校验和部署探针核对。
...
`mysql.log-level` 在所有模式下只接受 `1..4`。`release` 必须使用 GORM
silent、JSON、无颜色且关闭 development；任一值不满足都会在 MySQL/Redis
初始化前失败。`debug`/`test` 仍可显式使用 GORM info、console、color 和
development 进行本地调试。
...
注意：哪些依赖可以降级，必须以当前代码实现为准。没有代码支撑时，应标记为 `规划改造` 或 `设计目标`。
...
| YAML | 环境变量 | 生产值 | 开发值 |
| --- | --- | --- | --- |
| `health.readiness.component_timeout` | `IAM_APISERVER_HEALTH_READINESS_COMPONENT_TIMEOUT` | `500ms` | `500ms` |
| `health.readiness.total_timeout` | `IAM_APISERVER_HEALTH_READINESS_TOTAL_TIMEOUT` | `2s` | `2s` |
| `health.readiness.outbox_max_pending_age` | `IAM_APISERVER_HEALTH_READINESS_OUTBOX_MAX_PENDING_AGE` | `5m` | `5m` |
| `health.readiness.drain_delay` | `IAM_APISERVER_HEALTH_READINESS_DRAIN_DELAY` | `5s` | `0s` |

单组件超时必须小于总体超时。`/healthz` 是不访问依赖的 liveness，
`/readyz` 是接流量判定，兼容端点 `/health` 只返回脱敏汇总。
...
| YAML | 环境变量 | 生产值 | 开发值 |
| --- | --- | --- | --- |
| `auth.password_lockout.enabled` | `IAM_APISERVER_AUTH_PASSWORD_LOCKOUT_ENABLED` | `true` | `false` |
| `auth.password_lockout.threshold` | `IAM_APISERVER_AUTH_PASSWORD_LOCKOUT_THRESHOLD` | `5` | `5` |
| `auth.password_lockout.lock_duration` | `IAM_APISERVER_AUTH_PASSWORD_LOCKOUT_LOCK_DURATION` | `15m` | `15m` |

启用时要求 `threshold >= 1` 且 `lock_duration > 0`，否则在资源初始化前启动失败。本配置不提供 CLI flag。首次在生产启用前，需要先审查失败计数和有效锁定分布，再按受审查脚本
[reset-authn-password-lockout-baseline.sql](../../scripts/ops/reset-authn-password-lockout-baseline.sql)
清零没有有效锁定的历史失败计数。
...
| YAML | 生产值 | 开发值 |
| --- | --- | --- |
| `jwks.keys_dir` | `/app/data/keys` | ./configs/keys（运行时创建） |
| `jwks.auto_init` | `true` | `true` |
| `jwks.rotation.automatic_enabled` | `true` | `false` |
| `jwks.rotation.check_cron` | `@every 1h` | `@every 1h` |
| `jwks.rotation.rotation_interval` | `720h` | `720h` |
| `jwks.rotation.grace_period` | `168h` | `168h` |
| `jwks.rotation.max_publishable_keys` | `3` | `3` |

生产 `keys_dir` 必须是非空绝对路径。Cron、周期、grace 和发布数量在外部资源初始化前校验；环境变量使用
`IAM_APISERVER_JWKS_ROTATION_*` 前缀，不提供 CLI flag。当前私钥是 POSIX PEM，所有签名实例必须共享同一密钥目录。
...
手机号搜索必须考虑脱敏、限流和审计；
索引不可用时应有明确降级策略。
Suggest 索引只存在于进程内；`suggest.snapshot` 与 `suggest.data_dir` 已移除，旧 YAML 或对应环境变量会使启动失败。
```
...
当前 Identity Session 最终撤销配置契约：

| YAML | 环境变量 | 默认值 |
| --- | --- | --- |
| `identity.session_revocation.poll_interval` | `IAM_APISERVER_IDENTITY_SESSION_REVOCATION_POLL_INTERVAL` | `2s` |
| `identity.session_revocation.batch_size` | `IAM_APISERVER_IDENTITY_SESSION_REVOCATION_BATCH_SIZE` | `50` |
| `identity.session_revocation.retry_base_delay` | `IAM_APISERVER_IDENTITY_SESSION_REVOCATION_RETRY_BASE_DELAY` | `5s` |
| `identity.session_revocation.retry_max_delay` | `IAM_APISERVER_IDENTITY_SESSION_REVOCATION_RETRY_MAX_DELAY` | `5m` |
| `identity.session_revocation.stale_processing_after` | `IAM_APISERVER_IDENTITY_SESSION_REVOCATION_STALE_PROCESSING_AFTER` | `1m` |

该任务消费 MySQL 本地安全 Outbox 并幂等撤销 Redis Session，不依赖 NSQ，也不提供
exactly-once 保证。批量必须大于零，重试上限不得小于基础延迟。
