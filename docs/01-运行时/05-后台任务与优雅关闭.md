...
> 状态：设计目标 · 第一版正文，待继续按源码、process 生命周期、任务实现和部署配置核对细节。
...
Full 与 Delta 使用同一把非阻塞锁。任务重叠时后到任务跳过，不排队；成功游标使用查询开始时间，空 Delta 推进游标，Loader/Runtime 失败不推进。
...
5. initial load Suggest 进程内索引（如果配置要求启动前 ready）；
