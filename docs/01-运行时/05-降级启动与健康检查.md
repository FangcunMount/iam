...
- EventBus 不可用时，Outbox Relay 与 AuthZ Policy Sync Subscriber 会受到什么影响；
...
EventBus 不可用是一个重要的降级边界：outbox store 仍可能保留 pending/failed 事件，但 Outbox Relay 不完整或不启动，AuthZ Policy Sync Subscriber 也不会构建；授权版本事件不会通过 `iam.authz.version` topic 被本进程消费。
...
| 关键模块有哪些 | `idp`、`authn`、`authz`、`user`；其中 `user` 是运行时代码中的历史/实现命名，在文档语义中归入 Identity 能力边界。 | [../../internal/apiserver/container/runtime_deps.go](../../internal/apiserver/container/runtime_deps.go) |
| EventBus 不可用影响什么 | Outbox Relay 不完整或不启动，AuthZ Policy Sync Subscriber 不构建，授权版本事件不会被本进程从 `iam.authz.version` 消费。 | [../../internal/apiserver/container/runtime_deps.go](../../internal/apiserver/container/runtime_deps.go)、[../../internal/apiserver/process/shutdown_lifecycle.go](../../internal/apiserver/process/shutdown_lifecycle.go) |
...
但 EventBus 不可用会影响 Outbox Relay、AuthZ Policy Sync Subscriber 和异步事件传播：outbox store 仍可能保留 pending/failed 事件，但 relay 不完整或不启动，policy sync subscriber 不构建，授权版本事件不会通过 `iam.authz.version` topic 被本进程消费。
...
其中 `user` 是运行时代码中的历史/实现命名；在新版文档语义中，它归入 Identity 能力边界，负责 User、Profile、ProfileLink 相关 capabilities。
...
降级启动后，transport 仍会按模块能力条件注册路由，并且缺失安全能力时应 fail-closed。
...
这些诊断入口用于定位运行时状态，但不能单独证明所有业务能力完整可用。尤其是在降级启动场景下，应结合 `/debug/modules`、`/debug/routes`、启动日志和后台任务日志一起判断。
...
- EventBus ping；
- Outbox relay health；
- AuthZ policy sync subscriber health；
...
| `/api/v2/identity/me` 404 | 看 User module/Identity capability 和 JWT middleware 是否存在 |
...
| AuthZ/Identity/Suggest routes 不存在 | JWT middleware 不存在，通常是 AuthN TokenService 缺失；Identity 路由还依赖 User module/Identity capability |
### 13.5 授权版本事件没有被同步
...
```text
启动日志中 EventBus 创建是否成功
  -> Outbox Relay 是否启动
  -> AuthZ Policy Sync Subscriber 是否启动
  -> 是否订阅 iam.authz.version
  -> domain_event_outbox 中是否有 pending/failed/publishing 积压
```
...
| 现象 | 可能原因 |
| --- | --- |
| EventBus 为 nil | NSQ 未启用或创建失败 |
| outbox store 有 pending 事件，但 relay 不运行 | EventBus 不可用或 relay 未构建 |
| `iam.authz.version` 没有消费者日志 | AuthZ Policy Sync Subscriber 未构建或未启动 |
| 事件重复消费 | Outbox 是 at-least-once 语义，消费者需要按事件 id 或 tenant/version 幂等 |
...
| EventBus 不可用 | 记录 warning，继续 | outbox relay 不完整或不启动，AuthZ policy sync subscriber 不构建，授权版本事件不会通过 `iam.authz.version` 被本进程消费 |
...
| Layered Health | 不同层有不同健康语义 | `/health`、`/healthz`、gRPC health、`/readyz` | 运维侧不能混用单一探针，也不能把 liveness 当作业务 ready |
| Event Chain Degradation | EventBus 缺失时不误报完整事件链路 | relay 不完整或不启动，policy sync 不构建，outbox rows 保留 | 授权版本传播需要等待 EventBus 恢复或显式 reload |
...
```
### 第七轮：看 EventBus / Outbox / AuthZ Policy Sync
...
目标：搞清 EventBus 不可用时，Outbox Relay 与 AuthZ Policy Sync Subscriber 的降级边界。
...
另一个关键边界是 EventBus：EventBus 不可用不等于业务事实一定不能写入，但会影响 Outbox Relay、AuthZ Policy Sync Subscriber 和授权版本事件传播。排查授权变更不生效时，除了看 AuthZ reload health，也要看 outbox 积压、EventBus 状态和 `iam.authz.version` 订阅链路。
