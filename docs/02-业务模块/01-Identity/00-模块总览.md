...
这些选择不等于当前实现已经没有缺口。手机号唯一性、ProfileLink 撤销后重建、REST Profile 搜索范围、Block 与 Session 撤销的一致性等问题，仍需单独决策。
...
flowchart LR
    ...
    APP --> SESSION["AuthN SessionRevoker\nBlock only"]
...
- [ProfileLink 为什么不是 Permission](../../05-专题设计/05-ProfileLink为什么不是Permission.md)
- [Suggest 为什么是读模型](../../05-专题设计/06-Suggest为什么是读模型.md)
...
| 停用/封禁 User | 无 | `DeactivateUser`、`BlockUser` | 只有 Block 调用 SessionRevoker |
| 激活 User | 无 | 无 | application 存在 `Activate`，协议未暴露 |
| 查询 Profile | 当前用户列表、详情、相似搜索 | `GetProfile`、`BatchGetProfiles` | REST 详情/修改检查 active ProfileLink；搜索例外见缺口 |
...
| User Phone 可选，非空时唯一 | application checker | DB 无唯一索引，并发竞态未封口 |
...
### 9.1 已知缺口：手机号唯一性只有应用层预检查

`users.phone` 只有普通索引。两个并发事务可能都通过 `UniquenessChecker`，然后写入相同手机号。如果业务仍要求全局唯一，需要设计可空值的 DB 唯一约束和冲突错误映射。
...
### 9.4 已知缺口：REST Profile 搜索缺少关系或授权范围过滤

`GET /identity/profiles/search` 当前只有统一认证中间件，没有像 Profile 详情/更新那样检查 active ProfileLink，也没有注入 AuthZ/Suggest scope。在范围规则确定前，不应将它描述为“已完成的安全搜索”。

### 9.5 已知缺口：Block 与 Session 撤销不是原子操作

`Block` 先提交 Identity 数据库中的 blocked 状态，然后调用 AuthN `SessionRevoker`。如果撤销 Session 失败，User 仍已 blocked，但调用方收到错误。后续要根据安全需求决定重试、outbox 或会话校验时实时读状态等策略。

### 9.6 已知缺口：部分契约字段尚未落地

Identity proto 中的 `operator`、avatar、contacts、external identities 等部分字段，当前 handler/mapper 未消费或返回空值。它们是契约与实现的缺口，不是已实现的领域能力。

### 9.7 已知缺口：Suggest 派生数据可能包含已撤销关系

当前 Suggest 默认 Loader 读取 `profile_links` 时没有过滤 `revoked_at`，已撤销关系的 User 手机号可能继续进入 Profile 候选词。这是读模型派生链路的实现缺口，不改变 Identity 作为主事实源的边界。
...
| 数据库约束 | `internal/pkg/migration/migrations`、`configs/mysql/schema.sql` |
