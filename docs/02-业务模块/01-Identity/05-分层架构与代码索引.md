...
| Activate/Deactivate/Block | `internal/apiserver/application/identity/user/service_lifecycle.go` | Block 的 SessionRevoker 和提交后失败语义 |
...
| Phone 唯一性 | `internal/apiserver/domain/identity/user/uniqueness_checker.go` | DB 无唯一键，AuthN signup 未调用 |
...
| 更新 | `internal/apiserver/application/identity/profile/service_editor.go` | REST `MyProfiles.Patch` 另有 active-link 前置 |
| 查询 | `internal/apiserver/application/identity/profile/service_query.go` | REST search 当前无 scope 过滤 |
...
| `/identity/profiles/search` | `handler/profile_query.go` | `profile.Directory.FindSimilar` |
| `/identity/profile-links` | `handler/profile_link.go` | `profilelink.MyProfileLinks.List` |

REST 没有 User/Profile 创建或 ProfileLink 建立/撤销路由。OpenAPI 以 `api/rest/identity.v2.yaml` 为契约源。
...
`IdentityModule.CheckHealth()` 当前无条件返回 nil，没有探测 DB、repository 或跨模块端口。因此“Identity module health 通过”只表示当前模块健康方法未报错，不能当成运行时依赖真实可用的证据。
...
`configs/mysql/schema.sql` 当前仍将：

- `users.phone` 定义为 `NOT NULL`；
- `users.id_card` 定义为 `NOT NULL` 并建立 `uk_id_card`。

这与 migration `000003` 和 `000009` 执行后的终态不一致，也与当前 User domain/PO 不包含 IDCard 的模型不一致。

> 标签：已知缺口。在静态 schema 修正前，升级库的当前结构应以完整 migration 链的执行结果为准；任何直接用 `configs/mysql/schema.sql` 初始化的流程都需单独验证。
