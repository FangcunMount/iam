...
| --- | --- | --- |
| `/identity/me` | `handler/user.go`、`request/user.go`、`response/user.go` | User Editor/Directory + EffectiveRoleReader |
...
REST 没有 User/Profile 创建或 ProfileLink 建立/撤销路由。无作用域的 `/identity/profiles/search` 已下线；授权后的搜索入口是 `/suggest/profile`。
OpenAPI 以 `api/rest/identity.v2.yaml` 为契约源。
...
`internal/pkg/migration/migrations/*.sql` 是 schema 的唯一事实源。曾与 migration `000003`、`000009` 终态漂移的旧静态 schema 快照已移除；
新库必须执行完整 migration 链，随后才可按需重放不含 DDL 的 `configs/mysql/bootstrap.sql`。
