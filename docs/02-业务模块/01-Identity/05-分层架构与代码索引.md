...
`internal/pkg/migration/migrations/*.sql` 是 schema 的唯一事实源。曾与 migration `000003`、`000009` 终态漂移的旧静态 schema 快照已移除；新库必须执行完整 migration 链，随后才可按需重放不含 DDL 的 `configs/mysql/bootstrap.sql`。
