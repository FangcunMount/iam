...
AuthN 不回答“这个用户有什么权限”，也不保存完整用户档案。它回答四个连续问题：
...
这里每一步都缩小不确定性。OTP 正确不等于 User active；密码匹配不等于 Session 有效；JWT 签名正确也不等于业务操作已授权。
...
flowchart LR
    ...
    Principal --> Session["Redis Session"]
    Session --> Access["RS256 Access Token"]
    Session --> Refresh["Opaque Refresh Token"]
...
- `Principal`：本次认证成功后的运行时结果；
- `Session`：一段登录期的在线状态；
- AccessToken/RefreshToken：访问声明与续期能力。
...
`realm` 不是装饰字段。openid 只在一个 appID 内稳定，用户名可能只在租户内唯一；如果去掉 realm，不同应用/租户会错误合并身份。`GlobalIdentifier` 只在 provider 确实提供跨 realm 稳定 ID 时辅助查找，它不能被假设必有；同一 `(provider, GlobalIdentifier)` 只保存在一条 canonical LoginIdentity 上，同一 User 的其他 realm 行不重复保存该值。
...
外部 provider 调用不进入 MySQL 事务，避免网络尾延迟占用连接和锁。本地 User、LoginIdentity、Credential 通过同一个 UoW 原子提交。
...
- ProofFactory 处理 OTP/OAuth 外部交互，产出领域 credential；
- Authenticator 根据策略验证 LoginIdentity/Credential；
- CredentialRecorder 即使失败也要记录次数并执行锁定策略；
- AdmissionPolicy 再检查 User 与 LoginIdentity 是否允许建立认证状态；
- SessionEstablisher 创建 Session，并委托 TokenPairMinter 生成 token pair。
...
Session 支持按 session/user/login identity 撤销，`LifetimePolicy` 同时约束 refresh 滑动窗口和会话绝对上限。刷新会原子消费旧 refresh token并写入新 token，发现旧 token 已被消费时按 replay 处理并撤销 Session。

为什么不只用 JWT：纯 JWT 的优点是服务端无需查状态，缺点是 logout、User blocked、LoginIdentity disabled 和 refresh replay 只能等 token 过期。当前用一次在线状态查询换即时收敛。
...
| IssueToken | Session 后 mint/save refresh | save refresh 失败可能留下孤儿 Session |
...
把各 provider 直接写成独立 handler 容易复制状态检查和 token 签发。当前演化为 method selector + proof factory + domain authenticator，使协议差异、外部 I/O 和领域认证各自归位。
...
当前已经把 key metadata、rotation、publish snapshot 建模，但私钥仍为本地 PEM。更高安全级别可演进到 KMS/HSM 签名；届时 `JWTKeySource` 应返回 signer 能力而不是导出私钥，启动与轮换也需重做。
