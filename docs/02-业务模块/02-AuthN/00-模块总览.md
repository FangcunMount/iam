...
> 状态：设计目标 · 第一版正文，待继续按 `internal/apiserver/domain/authn`、`application/authn`、REST/gRPC 契约、Token/JWKS 实现和测试逐项核对。
...
```text
Session 不是 User 状态；
User blocked/inactive 会触发持久化的最终 Session 撤销，但二者不是同一个模型；
Session 不表达权限；
...
AuthN 登录时查询 User 状态；
Identity 在 User 被封禁或停用时，通过本地安全 Outbox 和受控 port 最终撤销 Session。
```
...
AuthN 不维护 ProfileSearchTerm / ProfileSuggestionIndex；
Suggest 不签发 Token。
```
