...
绑定的 `UserID` 必须来自当前 Principal，而不是信任请求体。不同输入实现自己的 `prepareLink`：手机号先消费绑定 scene 的 OTP，微信/企微先调用对应 IDP，之后统一进入 provider key 唯一性与 ensure 流程。
