...
- subject access：User 是否 active，LoginIdentity 是否 active；
- issue：建立在线 Session 并签发令牌。

### 失败为什么也要先 Record

密码猜测防护依赖连续失败记录。若 `decision.OK == false` 时立即返回，攻击路径恰好绕过锁定机制。因此当前顺序是先记录、再返回统一认证错误。

### 为什么认证成功还要 SubjectAccess

正确密码只证明秘密匹配，不代表被封禁 User 或已禁用 LoginIdentity 仍可进入系统。`SubjectAccessEvaluator` 同时读取两个状态，分别映射 blocked、disabled、inactive 等结果，并在登录、刷新、在线验证中复用。
...
当前全局唯一性检查和 ensure 最终仍必须依赖数据库唯一约束来处理并发；应用层预检查主要用于更清晰的错误，而不是并发正确性的最终来源。
