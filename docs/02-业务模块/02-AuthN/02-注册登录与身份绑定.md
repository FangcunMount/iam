...
正确密码只证明秘密匹配，不代表被封禁 User 或已禁用 LoginIdentity 仍可进入系统。`AdmissionPolicy` 同时读取两个状态，分别映射 blocked、disabled、inactive 等结果，并在登录、刷新、
在线验证中复用。它判断是否允许建立或维持认证状态，不负责 AuthZ 资源授权。
