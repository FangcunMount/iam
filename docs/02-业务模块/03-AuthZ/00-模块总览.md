...
> 状态：待补证据 · 第一版正文，待继续按:
> `internal/apiserver/domain/authz`、`application/authz`、Casbin runtime、Outbox/PolicyVersion 传播；
> REST/gRPC 契约和测试逐项核对。
...
读完本文后，应继续编写并阅读 AuthZ 的领域模型文档，把 `Subject`、`Resource`、`Action`、`Scope`、`Role`、`Permission`、`RoleBinding`、`PolicyVersion`、`AuthorizationDecision` 的模型和生命周期讲清楚。
