...
| Casbin RoleManager | 在内存中连接 Subject→Role、Role→Role | 否，可重建 |
| 原生授权索引 | 按 Tenant 和有效角色组织 Grant 并求值 | 否，可重建 |
...
Bearer user JWT
  ...
  -> RequirePermissionOrGlobal(resource, action)
  -> 当前 tenant AuthorizeRoute
  -> 必要时 platform AuthorizeRoute
...
`ProfileLink` 表达 User 与 Profile 的身份/监护关系，生命周期来自 Identity。把它复制为 Grant 会出现双写：关系解除后，Grant 是否同时撤销？失败时哪份为真？当前设计要求业务模块读取关系后形成对象属性或直接在业务域判断，不把关系行长期复制进 AuthZ。

### AuthN 状态不能变成长期角色

MFA、Session active、LoginIdentity enabled 是认证时态事实。它们变化频繁且有过期语义，不适合作为持久 Role。需要这类条件时，应在可信请求上下文中显式建模，并重新评估重放与缓存边界；当前 AuthZ v3 尚未把它们作为 ConstraintSet 输入。
...
优点是配置集中；代价是对象属性类型、schema、缺失属性、matched Grant、版本与管理写模型都要挤进字符串 policy。当前只复用 Casbin 擅长的角色图，不让工具反向定义领域语言。
...
仓库不再把权限规则持久化为 Casbin policy，也不存在运行时 Casbin 模型配置文件。删除 Casbin 前必须先提供等价、经过循环继承与多层继承测试的角色图实现；仅因为权限匹配已原生化，并不足以直接删除它。

当前 RoleManager 的最大继承层级配置为 32。快照构建前还会自行检测循环，因此 Casbin 接到的是已验证角色图；两层保护分别防止脏数据和运行时无界遍历。

## API 边界

- REST 管理前缀：`/api/v3/authz`。
