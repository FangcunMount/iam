...
AuthZ v3 在 reload 时从 MySQL 构建完整不可变快照，并以原子替换方式发布。请求期不查权限事实表：Casbin RoleManager 先算有效角色，原生运行时再匹配 PermissionGrant 并求值 ConstraintSet。
...
`MySQLSource.Load` 读取六类数据：Role、未删除 Assignment、未撤销 RoleInheritance、未撤销 PermissionGrant、Resource 与每个 Tenant 的 PolicyVersion。
...
只有所有数据都通过，新 Snapshot 才会进入 `atomic.Pointer`。任意一条错误都会让整次 reload 失败，而不是跳过脏数据继续发布。
...
```text
RoleManager(domain=tenant)
  subject-key -> role-key
  role-key    -> inherited-role-key
...
sequenceDiagram
  ...
  participant RM as Casbin RoleManager
  Caller->>GRPC: Subject, Tenant, Resource, Action, trusted attrs
  GRPC->>RT: Check
  RT->>RM: GetRoles(subject, tenant)
  RM-->>RT: direct + inherited roles
  RT->>RT: 收集候选 Grant
  RT->>RT: 匹配 Resource/Action
  RT->>RT: 校验 schema 并求值 ConstraintSet
  RT-->>GRPC: Decision + reason
...
如果提交 ObjectAttributes，必须同时提交 `object_id`。每个属性 key 必须使用 `object.*`，value 必须是唯一的类型值。gRPC transport 还会在进入领域请求前检查 caller/resource/key 白名单。

### 阶段 2：角色闭包

RoleManager 在 Tenant domain 中先取 Subject 的直接角色，再求全部可达 parent role。返回值去掉内部 `role:` 编码并排序。
...
找到第一条成功 Grant 后即返回 ALLOW，并记录 matched Grant ID 和 matched Role。如果条件仅因缺 key 不成立，运行时会汇总、去重、排序缺失 key；如果 key 存在但值不等，它是普通 no-match，不报错。
...
Decision 还包含求值时间。对外 proto 当前返回 allowed、reason、deny code、matched grant/role、policy version 与缺失 key，但不将整个候选 Grant 集合或内部图暴露给调用方。
...
`infra/authz/native/snapshot.go` 是当前唯一运行时 Casbin 入口。它向 RoleManager 装载两类边：
...
因此准确表述是“Casbin 是角色图计算组件”，不是“Casbin 是当前完整授权执行引擎”。

Casbin 的 domain 参数由 Tenant 提供，两类边都用内部编码区分 Subject key 与 Role key。如果未来移除 Casbin，必须先用自有图算法等价替换：
...
```text
新快照完整成功 -> atomic swap
新快照任一失败 -> old snapshot remains + health degraded
```
...
当前每个 ConstraintSet 最多 8 个 predicate，角色继承最大层次 32。它们是可控上界，但不等于经过任意规模生产压测。若 Grant 数量大幅增长，应使用实际角色/Grant 分布重新基准测试，而不是仅从无数据库查询推导为恒定成本。
...
- `internal/apiserver/infra/authz/native/snapshot.go`：角色图和不可变快照构建。
- `internal/apiserver/infra/authz/native/runtime.go`：原生 Grant/Constraint 求值。
- `internal/apiserver/application/authz/authorization`：`Check` 与快照查询。
- `internal/apiserver/transport/grpc/service/authz`：gRPC 映射。
- `internal/apiserver/infra/authz/native/runtime_test.go`：角色图、匹配和快照行为。

### 测试能证明什么

- domain constraint 测试证明规范化、类型校验、缺失与不等值的分流。
- native runtime 测试证明 Assignment+Inheritance 闭包、Grant 匹配、条件判定和 snapshot 合并语义。
