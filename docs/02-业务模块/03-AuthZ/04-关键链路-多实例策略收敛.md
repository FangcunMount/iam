...
Runtime 的可取消单一入口覆盖完整的加载、构建和发布过程。Check 不访问数据库，也不获取重载锁。发布检查比较每个已存在租户；其他租户的高版本不能掩盖某个租户回退。加载失败、构建失败、取消和回退拒绝均保留原快照。

事件只携带租户与版本，不携带策略增量。重复及乱序事件通过每租户最高目标版本去重；已加载的版本不重复触发全量读取。未收到消息的实例由数据库核对补偿。
...
| `iam_authz_native_loaded_policy_version_max` | 兼容保留的最大已加载版本；不能代替租户版本向量 |
| `iam_authz_native_policy_version_lag_max` | 每个租户的最高目标减已加载版本，再取最大差值 |
...
健康详情保留租户版本映射、最后事件、重载时间和 channel，增加 fresh、freshness_age_seconds、degraded、sync_running 和 subscription_registered。readiness 是实例就绪状态，集群收敛仍需逐实例比较版本。
...
如果数据库返回了低于已发布版本的租户版本，Runtime 保留现有快照并记录失败。运维应检查数据库路由、备份恢复或版本数据损坏；不能用降低内存版本的方式让健康检查恢复。
