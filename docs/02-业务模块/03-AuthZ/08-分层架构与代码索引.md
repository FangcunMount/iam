...
Domain 不反向依赖 Gin、gRPC、GORM、Redis 或 Casbin 具体配置。
```
...
flowchart TB
  subgraph Transport
    ...
    MW[JWT route middleware]
  end
  subgraph Application
    CMD[Role / Assignment / Grant / Resource / Inheritance commands]
    QRY[Authorization and catalog queries]
    SYNC[Policy version event handler]
  end
  subgraph Domain
  ...
  subgraph Infrastructure
    ...
    NATIVE[Native immutable runtime]
    CASBIN[Casbin RoleManager]
...
Casbin 只出现在 Infrastructure 的 native snapshot 构建内部。如果一个 domain/application 包开始直接引用 Casbin type，就表明执行引擎细节已向内泄漏。

## 代码索引

| 关注点 | 主要位置 |
|---|---|
| gRPC v3 契约 | `api/grpc/iam/authz/v3/authz.proto` |
...
| Assignment 写入 | `internal/apiserver/application/authz/rolebinding`、`assignmentauth` |
...
| 原生运行时与 Casbin 角色图 | `internal/apiserver/infra/authz/native` |
...
2. native Runtime 是否用 MySQLSource 初始建照。
3. authorization checker/snapshot reader/route authorizer 是否指向同一 runtime。
...
“包存在”不等于“运行时使用”。例如仓库中仍有 Casbin dependency，但它的实际入口只在 native snapshot 的 RoleManager；旧 evaluator 或旧 Casbin model 文件若未被组合根引用，就不能被文档写成活跃路径。
...
| 修改继承 | 循环检测、RoleManager 构图、snapshot/effective roles、测试 |
...
| 删除 Casbin | 先替换两类角色边闭包并补齐等价测试，再移除依赖与专题引用 |
...
需要同时评审传输凭据、`grpc_acl.yaml`、Assignment constraints、delegated actor 语义、受管 Role 所有权和并发写入模式。不能因为 caller 可调 `Check` 就顺便给它开 Assignment 写入。

### 修改 RoleInheritance

需要同时检查领域方向语义、数据层 active unique guard/锁、成环检查、snapshot 图编码、direct/effective roles 返回和最大层次。只把 UI 中的父子文案对调很容易将 `child -> parent` 写反。
...
架构测试还负责阻止 AuthZ v2 的退役 URL、旧 evaluator 和旧 Casbin 配置重新进入当前运行路径。

## 验证层次与能证明的结论

| 层次 | 代表检查 | 可证明 | 不可证明 |
|---|---|---|---|
| Domain 单测 | constraint/resource/rolebinding/inheritance tests | 构造与求值不变量 | 事务与生产数据 |
...
| Runtime 测试 | native snapshot/runtime tests | 图闭包、Grant/Constraint 判定 | 生产规模和滞后 SLO |
...
Casbin 当前只是 native snapshot 内的 RoleManager，因此可移除，但不是“删 go.mod 依赖”就完成。安全路径是：
...
5. 移除 Casbin import/dependency、退役配置门禁与相关文档，再做完整验证。

在第 3 步完成前，“Casbin 似乎只剩一处 import”不是可删除证据。
