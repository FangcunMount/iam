...
| Assignment 写入与准入 | `internal/apiserver/application/authz/assignment`、`assignmentadmission` |
...
| 策略变更与发布 | `internal/apiserver/application/authz/policychange`、`policypublication` |
...
“包存在”不等于“运行时使用”。当前组合根只装配自有 immutable-snapshot runtime；旧 evaluator、旧 Casbin model 或退役 adapter
