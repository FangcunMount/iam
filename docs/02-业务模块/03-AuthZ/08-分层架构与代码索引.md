...
自有角色图只出现在 Infrastructure 的 native snapshot 构建内部。Domain 只声明 `RoleResolver` 能力，不知道 map、遍历算法或快照布局。
...
| 原生运行时与不可变角色图 | `internal/apiserver/infra/authz/native` |
...
“包存在”不等于“运行时使用”。当前组合根只装配 native runtime；旧 evaluator、旧 Casbin model 或退役 adapter
即使仍出现在历史 migration 和退役门禁中，也不能被文档写成活跃路径。
