...
`ExternalIdentity` 是 IDP 内部的不可变请求内值对象，标识类型只允许 `open_id`、`union_id`、`user_id`、`open_user_id`，构造时检查 provider 对应不变量。它无序列化协议、仓储、缓存和重放入口；AuthN 只把它映射为既有认证输入或 LoginIdentity ProviderKey。外部标识在绑定到 LoginIdentity 之前不是 IAM User，也不能直接登录。
