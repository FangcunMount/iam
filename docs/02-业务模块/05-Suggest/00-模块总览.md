...
Identity MySQL facts
  ...
  -> Candidate recall + SelectionPolicy
  -> masked REST result
```
...
flowchart LR
    ...
    Loader --> Profile["SuggestibleProfile"]
    Profile --> TST["name / pinyin / initials TST"]
    Profile --> Hash["ProfileID / mobile Hash"]
    JWT["AuthN request context"] --> Principal["visibility.Principal"]
    AuthZ["profiles/list 与 search_by_mobile PermissionGrant"] --> FactsView["AuthorizationFacts"]
    Visibility["created_by resolver"] --> Resolver["ScopeResolver"]
    Principal --> Resolver
    FactsView --> Resolver
    Resolver --> Scope["visibility.Scope"]
    TST --> Recall["CandidateRecaller"]
    Hash --> Recall
    Recall --> Select["SelectionPolicy"]
    Scope --> Select
    Select --> Result["deduplicate -> rank -> limit -> mask"]
```

`SuggestibleProfile` 是受约束的派生读模型，只包含召回、可见性判断和排序所需的最小属性：ProfileID、DisplayName、Mobiles、Weight、OrgID、OwnerOperatorIDs。删除通过 `ProjectionChange` 表达，不伪装成空名称 Profile。
...
手机号不做前缀模糊搜索，能减少枚举面；纯数字也不会进入 TST。`MatchStrength` 把 Exact、DirectPrefix、ExpandedPrefix 带入排序，同权重时按该顺序优先。
...
查询会把短关键词补 `*` 到 KeyPadLen，通过三元搜索树展开最多 WildcardKeyCap 个终端 key，再最多收集 CandidateBudget 个候选。两个上限分别限制树展开和候选数量，避免短前缀造成无界扫描。
...
Store recall up to CandidateBudget
  -> SelectionPolicy filters unauthorized candidates
...
`visibility.Principal` 来自 JWT request context：

- OperatorID = authenticated UserID；
- TenantDomain = IAM 授权域；
- OrgID = 业务组织 claim；
- 数字形式的历史 tenant 标识会映射回默认业务域。

`queryprofile.ScopeResolverService` 读取授权事实和 visibility IDs，再由 `ResolutionPolicy` 组合：

1. platform 授权域的 `profiles/list` PermissionGrant -> `AllProfile=true`；
2. platform 或当前 tenant 授权域的 `search_by_mobile` PermissionGrant -> 手机号搜索能力；
3. Principal OrgIDs；
4. visibility resolver 按 `profiles.created_by=OperatorID` 补 ProfileIDs。

若 AuthZ runtime 为 nil，当前降级为 owner/org/visibility，而不是全量或报错；若 AuthZ 调用存在但返回错误，则整次查询报错。两者不能混为“AuthZ 不可用”。

当前实现不解析旧的超级管理员布尔标志，也不按 `tenant_admin` / `super_admin` 等角色名放行。平台全量 Profile 与手机号搜索都是 PermissionGrant 的结果；未获得平台
`profiles/list` 时，只组合 OperatorID、Principal OrgIDs 和 visibility resolver 的 ProfileIDs。
...
REST handler 在 scope/index 前按 operator 限流，普通关键词与手机号形态分桶。memory 是单实例 token bucket；Redis 是多实例共享的约 1 秒固定窗口。Redis 错误
fail-open，因此它是滥用抑制，不是强授权边界。
