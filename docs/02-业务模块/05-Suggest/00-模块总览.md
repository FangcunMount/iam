...
`ProfileSearchTerm` 同时包含搜索键和本地过滤的最小属性，不包含完整 Profile。当前字段为 ProfileID、DisplayName、Mobiles、Weight、OrgID、OwnerOperatorIDs，另有 deprecated TenantID 兼容字段。
...
OR deprecated TenantIDs compatibility
```
...
当前 tenant_admin/super_admin 只会合并 Principal 已有的 OrgIDs，不会凭角色自动产生全租户 Profile 范围；注释里旧的 TenantIDs 说法已经不代表实现。默认数据权限仍是过渡模型。
