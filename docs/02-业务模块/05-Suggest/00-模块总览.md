...
> 状态：规划改造 · 当前代码只提供 REST，返回类型为 application `ProfileSuggestItem`；本文中的 gRPC/SDK 与部分目标分层描述尚未实现。
...
- `ProfileSearchTerm`、`ProfileSearchIndex`、`ProfileAccessScope`、`ProfileSuggestItem` 分别表达什么？
...
最终返回的是 ProfileSuggestItem，不是 Profile 写模型。
```
...
### 5.4 ProfileSuggestItem

`ProfileSuggestItem` 是 Suggest 对外返回的候选项。
...
| 层 | 职责 |
| --- | --- |
| domain | 定义 ProfileSearchTerm / ProfileAccessScope / Query / ranking policy 等模型和不变量 |
...
| Suggest gRPC transport | 当前未提供 Suggest gRPC service |
...
读完本文后，应继续编写 Suggest 的领域模型文档，把 `ProfileSearchTerm / ProfileAccessScope / ProfileSuggestItem` 的模型和生命周期讲清楚。
