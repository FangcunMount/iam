...
当前实现不解析旧的超级管理员布尔标志，也不按 `tenant_admin`/`super_admin` 等角色名放行。平台全量 Profile 与手机号搜索都是 PermissionGrant 的结果；未获得平台 `profiles/list` 时，只组合 OperatorID、Principal OrgIDs 和 visibility resolver 的 ProfileIDs。
