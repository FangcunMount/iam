...
原始手机号仍会存在于 Loader 行、`SuggestibleProfile.Mobiles` 和 Hash key 中。
当前没有 snapshot 落盘，Suggest 专用手机号日志也不记录原始关键词，但 heap dump、调试接口和通用 access log 仍应按敏感数据标准治理。
...
当前 optional 启动降级不会启动后台重试，需通过重启恢复。
健康检查只确认 querier 非 nil 且至少成功刷新过一次，没有覆盖索引 age、Cron 活性、generation 一致性和 querier 是否已被替换为 degraded 的全部组合。
