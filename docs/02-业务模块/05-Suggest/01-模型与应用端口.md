...
Suggest 不拥有 Profile 生命周期。名称、手机号、关系状态的写入都发生在 Identity；Suggest 只能把这些事实投影成可重建状态。给 `SuggestibleProfile` 增加写仓储或主数据修改方法会制造双写和所有权冲突。
...
`Command` 携带领域 `Principal`、原始关键词和请求 limit；`ResultItem` 携带 ProfileID、DisplayName、MobileMask 和 Weight。REST 负责把 ProfileID 转成字符串并包统一 `{code,message,data}` envelope。
