...
| `SearchPolicy` / `SearchDecision` | domain | 统一 mobile_denied / numeric_exact / prefix_text |
| `CandidateSelectionPolicy` | domain | compile scope → filter → deduplicate → rank → limit |
| `ScopeResolutionPolicy` | domain | 根据授权事实与 visibility 生成 ProfileAccessScope |
| `ProfileIndexMutation` | domain | 显式 Upsert/Delete 索引变更 |
...
| `ProfileIndexSource` | application port | 从数据源执行 Full / Delta（Delta 返回显式 Mutation） |
| `ProfileAccessScopeProvider` | application port | 把 Principal 解析成查询 scope |
| `ProfileAuthorizationFactsReader` | application port | 查询 AuthZ 授权事实 |
...
| 字段 | 语义 |
| --- | --- |
| `OperatorID` | 当前已认证操作员 ID |
| `TenantDomain` | IAM 授权域，例如 `fangcun` 或 `platform` |
| `OrgID` / `OrgIDs` | 业务组织范围，不是 IAM tenant |

`OperatingPrincipal` 当前没有旧的超级管理员布尔字段。全量可见与手机号搜索分别由 AuthZ PermissionGrant 决定，不能从角色名称或客户端标志推导。
...
flowchart LR
    ...
    Refresher --> Source["ProfileIndexSource"]
    Refresher --> Runtime
```
...
- `ProfileSuggestionRuntime.Replace` 用于 Full；`ApplyDelta` 用于 Delta。
...
| 手机号形态且无权限 | `SearchPolicy` 决策 `mobile_denied`，返回空列表 |
...
| 查询服务和策略 | `internal/apiserver/application/suggest/service.go` |
