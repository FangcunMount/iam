...
`ProfileSuggestionIndex`、`ProfileSuggestionRuntime`、`SnapshotWriter` 和 `ProfileSuggestItem` 都是 application 符号，不是 domain entity。
...
| `SnapshotWriter` | application port | 可选写出刷新结果；当前不负责恢复 |
...
flowchart LR
    ...
    Refresher --> Runtime
    Refresher -. optional .-> SnapshotWriter
```
...
- `SnapshotWriter` 写失败只记录 warn，不回滚已经切换的 runtime。
