...
> 状态：待补证据 · 第一版正文，待继续按源码目录、契约文件、组合根、搜索运行时、可见性过滤、限流、指标、配置和架构测试逐项核对。
...
- `OperatingPrincipal / ProfileAccessScope / Query / ProfileSearchTerm / SuggestSnapshot / SuggestResult` 应该从哪些入口开始读？
...
```text
domain：表达 OperatingPrincipal / ProfileAccessScope / Query / ProfileSearchTerm / SuggestSnapshot / SuggestResult 的模型和不变量；
application：编排索引刷新、SuggestProfile 查询、可见性过滤、排序截断、脱敏、手机号安全策略等用例；
...
| gRPC transport | `../../../internal/apiserver/transport/grpc/suggest` | Suggest gRPC service、proto mapper，若已实现 |
| Container | `../../../internal/apiserver/container/suggest` | Suggest 模块装配 |
| REST 契约 | `../../../api/rest/suggest.v2.yaml` | Suggest REST OpenAPI 契约 |
| gRPC 契约 | `../../../api/grpc/iam/suggest/v2/suggest.proto` | Suggest gRPC proto 契约，若已存在 |
...
SuggestSnapshot 为什么不是 Profile 主数据？
SuggestResult 为什么必须脱敏？
...
| SuggestSnapshot | `../../../internal/apiserver/domain/suggest` | 搜索读模型快照，具体文件以源码为准 |
| SuggestResult | `../../../internal/apiserver/domain/suggest` | 脱敏返回结果，具体文件以源码为准 |
...
| 修改 SuggestResult 字段 | domain result、masker、REST/gRPC DTO、SDK |
...
| gRPC Suggest | `../../../internal/apiserver/transport/grpc/suggest` | Suggest gRPC service、proto mapper，若已实现 |
...
```text
../../../api/grpc/iam/suggest/v2/suggest.proto
```
...
3. domain/suggest：SuggestSnapshot / ProfileSearchTerm
