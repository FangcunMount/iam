...
1. AuthN 模块整体分层是什么；
2. 每一层负责什么、不负责什么；
3. User、LoginIdentity、Credential、Challenge、Principal、Session、Token、JWKS 分别落在哪一层；
4. Onboarding、Login、Linking、Challenge、Token、JWKS 的事实源文件在哪里；
5. 读代码、写文档、面试讲解时应如何定位事实源；
6. 如何避免领域模型、应用编排、Infra 实现互相污染。
...
这与 Ports & Adapters / Hexagonal Architecture 的思想一致：应用核心应通过接口与外部技术设施通信，使核心逻辑可以独立于 UI、数据库、外部服务进行开发和测试。

---

### 2.2 Domain 层是模型事实源

AuthN 的模型事实源在 Domain 层：
...
internal/apiserver/infra/idp 或实际 IDP adapter 包
```
...
| 主要职责 | 管理认证状态、refresh 生命周期、logout、revoke |
...
```text
1. Principal -> Claims -> JWS Token。
2. JWT 是 claims 表达。
3. JWS 是签名保护。
4. JWKS 只发布公钥。
5. KeyRotation 管理 active / grace / retired。
```
...
| 项 | 内容 |
| --- | --- |
| 职责 | 认证 LoginIdentity 控制权并签发 Token |
...
| --- | --- |
| `sign_in.go` | 登录主编排 |
| `types.go` | LoginCommand / LoginResult 等类型 |
| `method` | 登录方式选择 |
| `proof` | 构造领域认证凭据 |
| `reauth` | 重新认证支持 |
| `auth_failure.go` | 认证失败语义 |

领域策略：

| 策略 | 文件 |
| --- | --- |
| Password | `domain/authn/authentication/auth-password.go` |
| Phone OTP | `domain/authn/authentication/auth-phone-otp.go` |
| WeChat Mini | `domain/authn/authentication/auth-wechat-mini.go` |
| WeCom | `domain/authn/authentication/auth-wechat-com.go` |
...
## 5.5 Token / Session / JWKS

| 能力 | 事实源 |
| --- | --- |
| Token 签发 | `internal/apiserver/application/authn/token` |
...
```text
Principal -> TokenApplicationService -> JWT/JWS -> TokenStore / SessionManager
```
...
3. 创建 Application 服务：Onboarding、Login、Linking、Challenge、Token、JWKS。
4. 注入 Transport Handler。
```
...
```text
AuthN 产出 Principal。
AuthZ 使用 UserID + Scope + Role/Permission 做授权判断。
```
...
| `02-Login链路-从登录请求到Principal与Token.md` | 登录认证与 Token 签发 |
| `03-Linking链路-登录身份绑定解绑与安全边界.md` | 已认证 User 绑定/解绑 LoginIdentity |
| `04-Challenge链路-短信验证码与短期认证挑战.md` | OTP / 短期认证挑战 |
| `05-Session与Token边界-Principal-Session-JWT-RefreshToken.md` | 认证结果、会话、Token 边界 |
| `06-JWT-JWS-JWKS与KeyRotation.md` | JWT/JWS/JWKS 与密钥轮换 |
...
```text
先读 00，建立模型；
再读 01/02/03/04，理解核心链路；
再读 05/06，理解 Token/JWKS；
再读 07，理解外部 IDP；
最后读 08，回到代码事实源。
```
...
internal/apiserver/application/authn/login/method
internal/apiserver/application/authn/login/proof
...
如果是业务账号，应放在业务系统或 AuthZ scope/role 中。
...
> IAM 的 AuthN 模块采用分层架构组织：Transport 层只做 REST/gRPC 协议适配，Application 层负责编排 Onboarding、Login、Linking、Challenge、Token、JWKS 等用例，Domain 层表达 User、LoginIdentity、Credential、Challenge、Principal、Session 等认证模型，Infra 层实现 MySQL、Redis、JWT/JWS、JWKS、短信、微信/企微 IDP 等技术细节。这样可以把认证语义和技术实现解耦，避免 Handler 直接操作数据库，也避免领域模型依赖 JWT、GORM 或 Redis。

进一步可以补充：

> 这个分层最重要的收益是模型稳定。LoginIdentity 是登录身份绑定，Credential 是长期认证材料，Challenge 是短期认证挑战，Principal 是认证成功后的主体表达。JWT/JWS/JWKS 是 Token infra 层的安全表达，不属于领域模型。通过代码事实源索引，可以快速定位每条认证链路的应用层、领域层和基础设施实现。
...
3. 让 MySQL、Redis、JWT、IDP adapter 可以替换；
4. 让文档和代码都能围绕事实源维护。
```
