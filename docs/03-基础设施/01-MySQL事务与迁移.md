...
`000020` 单独退役冗余的 `schema_version`，已在生产验收 `version=20, dirty=0` 且旧表不存在。`000021` 在当前 bootstrap 停止写入后退役 `tenants/data_dictionary`；本文件随仓库实现更新时，目标库仍停在版本 20。两者都先检查数据库对象依赖，down fail closed；AuthN 旧表仍受只读 parity gate 约束，当前没有 `000022`。
