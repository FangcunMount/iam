...
为什么 RoleBinding 必须带 Tenant？
为什么当前 group/service 是模型预留但写入侧主要开放 user？
```

本文不展开 Resource / Action / Scope 的细节，也不展开 Permission 写入事务、Casbin p/g facts、PolicyVersion、Outbox。
...
Assignment   REST / proto 对外 wire term，内部标准术语仍是 RoleBinding
```
...
Role 的关键不是 ID，而是 `RoleName + TenantID`。
...
因为 RoleName 被 Permission、RoleBinding、Casbin facts、Snapshot 等链路引用。
...
### 15.3 RoleBinding 就是 Permission
...
### 15.4 Assignment 和 RoleBinding 完全一样
...
### 15.7 直接操作 Casbin g fact 就可以管理角色绑定
...
| 主题 | 事实源 |
| --- | --- |
| SubjectRef | `domain/authz/subject` |
| TenantID | `domain/authz/tenant` |
| Role / RoleName | `domain/authz/role` |
| RoleBinding / Binding / Fact | `domain/authz/rolebinding` |
| SubjectResolver | `domain/authz/rolebinding` |
| Role command | `application/authz/role` |
| RoleBinding command | `application/authz/rolebinding` |
| PolicyAdministration bind/unbind | `application/authz/policy` |
