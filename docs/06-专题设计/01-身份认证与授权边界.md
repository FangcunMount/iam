...
AuthZ 把 Principal 映射为 Subject，再用当前 runtime policy 对 Resource/Action/Scope 判定。详见 [AuthZ 授权模型](../02-业务模块/03-AuthZ/01-授权模型与匹配语义.md)。

## 4. ProfileLink 为什么不是 Permission

ProfileLink 是 Identity 拥有的用户—档案业务关系，回答“User 与这个 Profile 有什么关系”；Permission 回答“某 Subject 在某 Tenant 能否执行某 Action”。

关系事实可以作为授权输入，但不能直接等价：

- 同一 ProfileLink 关系对 read/update/export 的权限不同；
- 权限还受 tenant、role、scope、资源和当前状态约束；
- Identity 事务不应直接写 Casbin policy 代替业务关系；
...
- 每次敏感操作由 AuthZ Check 当前 policy。

本地 JWKS verify 只做签名/claims，存在状态撤销窗口；它适合低延迟路径，但高风险操作应选在线语义或足够短 token TTL。

## 6. 多租户为何贯穿身份和权限

Principal 可携带 TenantID，AuthZ policy 使用 tenant/domain 隔离；但 tenant 必须由可信上下文确定。若客户端可随意选 tenant，即使 Casbin matcher 完全正确也可能在错误域中做判定。
...
| authorization power | 在当前上下文能执行什么 | Subject、RoleBinding、Permission、Resource、Action、Scope | grant、revoke、策略 reload |
...
authorization power = 仍持有某些 RoleBinding
```
...
把 OTP code 写进 Credential 会混淆一次性消费和长期轮换；把 password hash 写进 User 又让 Identity 被迫理解认证算法。当前拆分使 password、OTP、provider proof 共享“产生 Principal”的主线，但各自拥有不同 verifier 和防重放策略。
...
ProfileLink(User, Profile, relation)
    说明业务关系
    ↓ 可作为可信事实输入
ScopeResolver / policy rule
    把关系翻译为某类对象范围
    ↓
Check(Subject, Tenant, Resource, Action, Scope)
    由 RoleBinding + Permission 得到 allow/deny
```
...
- AccessToken 被加入撤销标记；
- RoleBinding/Permission 被 revoke；
...
| 当前 Role/Permission | 不检查 | 不负责资源授权 | 检查当前 runtime policy |
...
工程上可以串联，语义上必须保留阶段：先建立可信 Principal，再构造 Subject/Tenant/Resource/Action/Scope，最后 Check。阶段分离才能正确映射 Unauthenticated 与 PermissionDenied，也能独立度量 token failure 和 policy deny。
...
3. Tenant 和 Scope 必须由服务端可信上下文建立；
4. ProfileLink 不直接写成 Casbin g/p fact；
...
不是。所有权只是授权输入，还需按 action、scope、tenant 和业务状态定义政策。尤其导出、分享、删除等动作通常需要额外权限。
...
```bash
/Users/yangshujie/.gvm/gos/go1.25.12/bin/go test ./internal/apiserver/domain/... ./internal/apiserver/application/... ./internal/apiserver/container/...
```
