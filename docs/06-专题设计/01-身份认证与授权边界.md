...
AuthZ 把 Principal 映射为 Subject，再用当前原生不可变快照对 Resource/Action 和受信 ObjectAttributes 判定。详见 [AuthZ 授权模型](../02-业务模块/03-AuthZ/01-授权模型与匹配语义.md)。

## 4. ProfileLink 为什么不是 PermissionGrant

ProfileLink 是 Identity 拥有的用户—档案业务关系，回答“User 与这个 Profile 有什么关系”；PermissionGrant 回答“某 Role 对 Resource/Action 是否拥有无条件或对象条件 allow”。Subject 是否获得该能力还需要 Assignment 和 RoleInheritance。
...
- 每次敏感操作由 AuthZ Check 当前原生 runtime snapshot。

本地 JWKS verify 只做签名/claims，存在状态撤销窗口；它适合低延迟路径，但高风险操作应选在线语义或足够短 token TTL。

## 6. 多租户为何贯穿身份和权限

Principal 可携带 TenantID，AuthZ 的 Assignment、RoleInheritance、PermissionGrant 和运行时角色图都按 tenant/domain 隔离；但 tenant 必须由可信上下文确定。若客户端可随意选 tenant，即使 Grant 求值完全正确也可能在错误域中做判定。
...
| 当前 Assignment/RoleInheritance/PermissionGrant | 不检查 | 不负责资源授权 | 检查当前原生 runtime snapshot |
...
4. ProfileLink 不直接写成 Assignment、PermissionGrant 或 Casbin role link；
