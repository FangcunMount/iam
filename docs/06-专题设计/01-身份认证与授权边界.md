...
AuthZ 把 Principal 映射为 Subject，再用当前 runtime policy 对 Resource/Action 和受信对象属性判定。详见 [AuthZ 授权模型](../02-业务模块/03-AuthZ/01-授权模型与匹配语义.md)。
...
工程上可以串联，语义上必须保留阶段：先建立可信 Principal，再构造 Subject/Tenant/Resource/Action/ObjectAttributes，最后 Check。阶段分离才能正确映射 Unauthenticated 与 PermissionDenied，也能独立度量 token failure 和 policy deny。
