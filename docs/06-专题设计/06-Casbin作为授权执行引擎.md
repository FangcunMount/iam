...
> 状态：已实现 · IAM 已用原生 PermissionGrant runtime 替换通用权限 matcher；Casbin 的唯一职责是内存 RoleInheritance 计算。
...
| Casbin role manager | 进程内计算直接及继承 Role |
| transport/application | 建立可信 Subject/Tenant/Resource/Action/ObjectAttributes |

角色继承是成熟、封闭的图计算问题，保留 Casbin role manager 可以复用其 domain-aware 继承能力。PermissionGrant 条件则是 IAM 自己的版本化契约，需要类型、Schema、错误码、审计标识和快照模式，适合由领域与原生 runtime 显式实现。

## 2. 为什么不再让 Casbin执行权限规则
