...
- `make api-validate` 在本地和 CI 执行固定版本 Spectral、schema 对齐、Swagger/OpenAPI 路由对齐和 Gin/OpenAPI 路由对齐；
...
>
> 它由一个 apiserver 进程同时提供 REST 和 gRPC，Go SDK 主入口走 gRPC。最重要的边界是 ProfileLink 不是 Permission、JWT 验签不是授权、Casbin 是 runtime 而不是领域模型、Suggest 候选不是访问凭证。当前 Suggest 还没有 gRPC/SDK；OpenAPI 已进入 CI，proto 生成仍需按发布流程核验。
...
| 所有契约检查都已进 CI | CI 已跑 docs hygiene/docs facts、`api-validate`、lint、全仓 race test 和 build；proto 生成仍未独立接入 |
| Suggest Full 和 Delta 都原子切换 | Full 新建 Store 后原子替换，Delta 在当前 Store 内互斥更新 |
| Suggest 有本地文件恢复事实源 | 当前只保留进程内索引，重启从 MySQL Full 重建 |
