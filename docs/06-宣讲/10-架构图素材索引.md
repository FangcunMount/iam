...
> 状态：设计目标 · 宣讲第一版，已按 06-宣讲目录重新整理架构图素材；后续需要继续结合当前 Mermaid 图、README、业务模块文档、接入契约、架构护栏和实际演示材料逐项校准。
...
```text
Suggest 从 Identity 的 Profile facts 派生 ProfileSearchTerm 和 ProfileSuggestionIndex。查询时 keyword 先命中候选，但候选不能直接返回，必须先做可见性过滤，再排序、截断和脱敏。手机号搜索只能扩大匹配方式，不能扩大可见范围，返回结果只能是脱敏候选。
```
...
讲 Suggest 时放 ProfileSearchTerm / ProfileSuggestionIndex / VisibilityFilter / ProfileSuggestItem。
```
...
../02-业务模块/05-Suggest/01-模型与应用端口.md
```
...
Suggest 当前模型主文档是 01-模型与应用端口.md；
