...
> 状态：待补证据 · 宣讲第一版，已按项目定位、系统架构、五个业务模块、专题设计、接入契约、工程护栏和 Verify 证据重新整理；后续需要继续结合当前代码路径、契约文件、测试结果和 README 逐项核对。
...
```text
因为 Suggest 不维护 Profile 主数据，只从 Identity 的 Profile facts 派生搜索 token 和 SuggestSnapshot，用于 keyword 搜索、排序、可见性过滤和脱敏展示。它可以重建、可以最终一致、可以保守降级，不应该污染 Identity 的写模型和关系不变量。
```
...
```text
不是。搜索命中只是候选召回，必须先做 ProfileAccessScope 和可见性过滤，再脱敏返回 SuggestResult。即使候选可见，也不代表有详情读取、修改或导出权限。后续详情接口仍然要用 Resource、Action、Scope 重新走 AuthZ Check。
```
...
```text
手机号搜索只能扩大匹配方式，不能扩大可见范围。实现上应该对手机号 keyword 做归一化、hash 或后缀 token 匹配，配合额外权限或策略、限流、审计、可见性过滤，并且响应只返回 mobile_mask，不返回明文手机号。
```
