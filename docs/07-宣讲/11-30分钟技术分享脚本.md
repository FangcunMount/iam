...
> 本文负责把 IAM 的项目定位、业务背景、系统架构、AuthN、AuthZ、Identity、IDP、REST/gRPC/SDK、Suggest 辅助读模型和工程护栏，组织成一场 30 分钟技术分享。
...
```text
Go IAM 服务实践：Session/Token、资源授权、ProfileLink、Suggest 与契约治理
```
...
5. Suggest 如何作为辅助读模型服务后台 Profile autocomplete；
6. 项目如何通过架构测试、契约测试、SDK compile test、Suggest tests、docs-hygiene 支撑长期演进。
```
...
| 5:00 - 8:30 | 系统架构总览 | 讲分层、模块、运行时装配和 Suggest 定位 |
| 8:30 - 13:30 | AuthN 认证体系 | 讲 Principal、Session、Token、JWKS/Verify |
| 13:30 - 18:30 | AuthZ 授权体系 | 讲模型、Check、写入、Outbox |
| 18:30 - 21:30 | Identity/ProfileLink | 讲 User、Profile、ProfileLink 及其与 Suggest 的边界 |
| 21:30 - 23:30 | IDP 与第三方登录 | 讲 IDP/AuthN 边界 |
| 23:30 - 25:30 | REST/gRPC/SDK 接入 | 讲接入方式、SDK 定位和 Suggest REST |
| 25:30 - 27:30 | Suggest 辅助读模型 | 讲 ProfileAccessScope、Trie/Hash、mobile_mask 和降级 |
| 27:30 - 29:00 | 工程质量与架构护栏 | 讲自动化护栏和防漂移 |
| 29:00 - 30:00 | 总结与 Q&A 引导 | 收束亮点，导向追问 |
...
这个项目最容易被误解成“用户中心”或者“登录系统”，但它的定位更大一些。它不是只维护 users 表，也不是登录后发一个 JWT，而是面向业务系统接入，统一处理登录认证、Session 与 Token 管理、资源授权判定、用户和业务档案关系、第三方身份源集成、REST/gRPC/SDK 接入，以及管理后台 Profile autocomplete 这类辅助读模型能力。

一句话说，IAM 解决的是：在多入口、多服务、多身份源的业务系统里，谁能登录、登录态现在是否有效、能访问什么资源、和哪些业务档案有关，其他业务系统如何稳定接入这些能力，以及后台如何快速、安全地搜索当前可见的 Profile 候选。
```
...
```text
IAM = AuthN + AuthZ + Identity + IDP + Access Contract + Suggest + Guard
```
...
第五，管理后台 Profile 查询容易越界。后台经常需要 autocomplete 快速定位档案，但如果直接让前端查 MySQL、或者接口只做关键词匹配不做权限过滤，就可能出现 A 能搜到不该看的 Profile，甚至通过手机号搜索泄露隐私。

所以 IAM 的价值是把这些能力收敛成统一基础服务：AuthN 管登录态，AuthZ 管访问权，Identity 管身份档案关系，IDP 管外部身份源，对外通过 REST/gRPC/SDK 接入，并通过 Suggest 辅助读模型服务管理后台 Profile autocomplete。
```
...
- admin autocomplete may leak profiles/mobile
...
```text
IAM:
AuthN / AuthZ / Identity / IDP / REST-gRPC-SDK / Suggest / Guard
```
...
纵向上，它采用分层和六边形架构。外部请求通过 REST、gRPC 或 SDK 进入系统，Transport 层只做协议适配，比如路由、参数绑定、错误映射和 middleware。真正的用例编排在 Application 层，比如登录、Verify、AuthZ Check、授权写入、ProfileLink 操作、Suggest 查询和索引刷新。Domain 层承载业务规则，比如 Session 是否 active、RoleBinding 是否成立、Permission 是否覆盖某个 action、ProfileLink 是否重复、ProfileAccessScope 如何过滤 Suggest 候选。Infra 层适配 MySQL、Redis、Casbin、JWT、Outbox、微信/企微 API，以及 Suggest 的 Trie/Hash 搜索索引运行时。

运行时上，cmd/apiserver 只是入口，process 负责进程生命周期，比如配置、资源准备、服务启动、后台任务和优雅关闭。container 是组合根，负责装配 AuthN、AuthZ、Identity、IDP、Suggest 等模块，并把能力投影给 REST deps、gRPC registration 和 runtime tasks。

横向模块上，主要是四个核心边界：AuthN 负责认证态，AuthZ 负责访问权，Identity 负责用户和业务档案关系，IDP 负责外部身份源基础设施。除此之外，IAM 内置了 Suggest 辅助读模型，用于 operating 后台 Profile autocomplete。它不是 IAM 核心身份域，也不是完整搜索服务，而是一个读模型和工程亮点。
```
...
```text
AuthN    AuthZ    Identity    IDP    Suggest(辅助读模型)
```
...
Suggest 返回 mobile_mask，不返回明文 mobile。
```
...
## 11. Identity 与 ProfileLink：18:30 - 21:30
...
新增 Suggest 后，还要补一个边界：ProfileLink 也不是 Suggest 的 ProfileAccessScope。ProfileLink 可以成为构建可见性读模型的事实来源之一，但 Suggest 查询时消费的是 ProfileAccessScope，并在 Trie/Hash 召回结果上做 scope filter。不要让 Suggest 每次 autocomplete 直接扫描 ProfileLink，也不要把“用户和档案有关系”直接讲成“当前操作员能搜索所有相关档案”。
```
...
```text
User 是登录主体，Profile 是业务档案，ProfileLink 是关系事实；ProfileAccessScope 才是 Suggest 查询可见范围。
```
...
## 13. REST / gRPC / SDK 接入：23:30 - 25:30
...
Suggest REST 也属于管理端 REST 接入的一部分，例如 GET /api/v2/suggest/profile。它服务 operating 后台 Profile autocomplete，不是完整搜索服务。这个接口必须根据 OperatingPrincipal 解析 ProfileAccessScope，召回候选后做 scope filter，手机号形态关键词需要额外授权，响应只返回 mobile_mask。
...
```text
REST 服务前端、管理面和 Suggest autocomplete，gRPC 服务可信服务间调用，SDK 服务 Go 业务系统低成本接入。
```
...
```text
刚才提到 Suggest REST，下面单独用两分钟讲清它为什么不是一个简单 SQL 查询。
```

---

## 14. Suggest 辅助读模型：25:30 - 27:30

### 14.1 讲稿

```text
Suggest 是这次文档更新后需要单独补充的一个工程亮点。

从领域边界上讲，它不是 IAM 的核心身份域，未来独立服务也有合理性。但当前阶段，如果把它拆成独立服务，会额外引入部署、鉴权、数据同步、观测和运维成本。因此它暂时保留在 iam-apiserver 内，作为 Profile 联想搜索辅助读模型，服务 operating 后台 autocomplete。

它解决的问题不是“搜索所有 Profile”，而是“当前操作员输入关键词时，快速返回他有权看到的 Profile 候选”。所以 Suggest 的核心不是 Trie 本身，而是性能和安全边界同时成立。

查询链路大概是这样：请求进入 GET /api/v2/suggest/profile 后，REST handler 提取当前 OperatingPrincipal；application service 通过 ProfileAccessScopeProvider 解析 ProfileAccessScope；然后从 ProfileSuggestionRuntime 当前活动索引中用 Trie/Hash 召回候选；候选必须先经过 scope filter，再排序；最后响应 DTO 只返回 mobile_mask，不返回明文手机号。

索引模型上，ProfileSearchTerm 是为了搜索构建的读模型投影，不是 Profile 聚合本体。Trie 或 TST 适合姓名、拼音、简拼前缀匹配，Hash 适合 ProfileID 和手机号这类精确匹配。刷新链路上，Full refresh 构建完整 store 并原子替换 Runtime，Delta refresh 导入变化 term 或 tombstone，Full refresh 负责定期兜底，避免旧 key 残留。

安全上，手机号搜索是高敏感能力，需要 AllowMobileSearch、限流、日志脱敏和指标观测。Suggest 如果索引不可用，在非强依赖场景下可以降级为空结果；这比绕过权限直接查全局 MySQL 更安全。

所以 Suggest 的定位是：它不是完整搜索服务，不是 AuthZ 权限中心，也不是 Identity 核心域；它是 IAM 内置的 Profile autocomplete 读模型，用 ProfileAccessScope、Trie/Hash、Full/Delta refresh、mobile_mask 和降级策略，解决后台高频查询的体验与安全问题。
```
...
flowchart TD
    ...
    Service --> Runtime --> Store --> Filter --> Result
    Scope --> Filter
```
...
```text
Suggest 不是搜索所有 Profile，而是在当前操作员的 ProfileAccessScope 内做低延迟 autocomplete。
```
...
```text
最后，IAM 作为基础服务，还需要一套护栏保证这些边界长期不漂移。
```

---

## 15. 工程质量与架构护栏：27:30 - 29:00
...
第四是 Suggest 专项护栏。Suggest 不能绕过 ProfileAccessScope，不能把 ProfileLink 写成可见范围，不能返回明文 mobile，不能在降级时直接查全局 MySQL。Suggest REST 契约要和 api/rest/suggest.v2.yaml、transport/rest/suggest、docs/08-Suggest 保持一致。

第五是 docs-hygiene。它检查活跃文档链接和退役路径、旧路由、旧术语，避免旧事实回流。也就是说，文档也被当成工程资产管理。
```
...
```text
这些护栏不是洁癖，而是为了让 IAM 作为基础服务长期可维护，也为了防止 Suggest 这类辅助读模型越界成权限中心或隐私泄露点。
```
...
第六，Suggest 作为内置辅助读模型服务管理后台 Profile autocomplete，用 ProfileAccessScope、Trie/Hash、Full/Delta refresh、mobile_mask 和降级策略同时解决体验、性能和安全边界。

第七，项目用架构测试、契约测试、SDK compile test、Suggest tests 和 docs-hygiene 保护边界，防止长期演进中漂移。

如果用一句话收束：这个项目的价值不是写了几个登录和权限接口，而是把身份、认证、授权、档案关系、第三方身份源、系统接入和后台辅助查询做成了一个可演进、可治理、可验证的 Go IAM 服务。
```
...
3. Suggest 里的 ProfileAccessScope / Trie / Hash / Full / Delta / mobile_mask；
4. 工程质量里的架构测试和契约防漂移。
```
...
架构上采用分层与六边形架构，Transport 负责 REST/gRPC 协议适配，Application 编排用例和事务，Domain 承载业务规则，Infra 适配 MySQL、Redis、Casbin、JWT、Outbox、微信/企微 API 和 Suggest Search，Container 作为组合根，Process 管生命周期。
...
对外通过 REST、gRPC、SDK 接入，同时通过 Suggest REST 服务管理后台 Profile autocomplete。Suggest 是辅助读模型，不是核心身份域；它通过 ProfileAccessScope、Trie/Hash、mobile_mask 和降级策略兼顾体验与安全。最后，项目通过架构测试、契约测试、SDK compile test、Suggest tests 和 docs-hygiene 防止长期演进中边界漂移。
```
...
ProfileLink 和 ProfileAccessScope 区别？
怎么防止用户访问别人 Profile？
```
...
```text
Suggest 为什么放在 IAM 里，而不是独立服务？
Suggest 为什么不直接查 MySQL？
ProfileAccessScope 和 ProfileLink 有什么区别？
如何保证 Suggest 不返回不该看的 Profile？
手机号搜索为什么只返回 mobile_mask？
Full refresh 和 Delta refresh 怎么分工？
Suggest 挂了为什么可以降级为空结果？
```
...
SDK：不是业务层，而是接入产品层。
Suggest：不是搜索服务或权限中心，而是 Profile autocomplete 辅助读模型。
Guard：不是测试多，而是边界可验证。
```
...
| Suggest | `docs/08-Suggest/README.md`、`docs/08-Suggest/01-查询链路-SuggestProfile从请求到索引过滤.md`、`docs/08-Suggest/02-权限范围-OperatingPrincipal与ProfileAccessScope.md`、`docs/08-Suggest/03-索引模型-ProfileSearchTerm-Trie-Hash-Runtime.md`、`docs/08-Suggest/04-刷新链路-Loader-Refresher-FullDelta-Snapshot.md`、`docs/08-Suggest/05-安全与运维-手机号搜索-限流-指标-降级.md` |
...
| Suggest 机器契约 | `api/rest/suggest.v2.yaml` |
| Suggest 代码事实源 | `internal/apiserver/transport/rest/suggest`、`internal/apiserver/application/suggest`、`internal/apiserver/domain/suggest`、`internal/apiserver/infra/suggest`、`internal/apiserver/infra/mysql/suggest` |
...
为什么需要 IAM
  ...
  -> Suggest 如何服务后台 Profile autocomplete
  -> 架构护栏如何保护长期演进
```
...
```text
IAM 不是普通用户中心，而是一个围绕认证、授权、身份关系、第三方身份源、业务系统接入和后台辅助查询构建的 Go 身份与访问管理基础服务。
```
...
IDP 管外部身份源；
REST/gRPC/SDK 是接入层；
Suggest 是 Profile autocomplete 辅助读模型；
Outbox 管授权版本传播；
架构测试、契约测试和 Suggest tests 管长期边界。
```
