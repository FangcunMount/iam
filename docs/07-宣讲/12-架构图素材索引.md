# 12-架构图素材索引

## 1. 本文定位

本文是 `07-宣讲/` 的架构图素材库。

它不是连续阅读的技术文章，也不替代事实层文档。

它服务于：

```text
技术分享 PPT；
面试白板讲解；
简历项目答辩；
README / docs 插图；
Notion 项目复盘；
30 分钟技术分享脚本配图。
```

本文回答：

```text
IAM 项目可以画哪些图？
每张图适合回答什么问题？
每张图适合放在哪篇宣讲文档里？
面试时应该优先准备哪几张图？
技术分享时应该按什么顺序展示图？
Suggest 辅助读模型应该画成什么图，避免讲成核心身份域或完整搜索服务？
每张图怎么讲才不变成“复杂图表堆砌”？
```

一句话：

> 本文负责把 IAM 的项目定位、系统架构、AuthN、AuthZ、Identity、IDP、接入体系、Suggest 辅助读模型和工程护栏，整理成可复用、可白板、可宣讲、可追问的架构图素材。

---

## 2. 使用原则

架构图不是越多越好。

推荐原则：

```text
1. 一张图只回答一个问题；
2. 面试最多准备 3～6 张核心图；
3. 30 分钟技术分享最多展示 8～12 张图；
4. 图要服务讲法，不要为了复杂而复杂；
5. 图里的每条箭头都要能解释；
6. 图必须能回链事实层文档、源码、OpenAPI、proto、SDK 或测试；
7. 不画已经归档、已经废弃或你讲不清楚的旧结构；
8. Suggest 可以作为工程亮点画图，但不能画成 IAM 核心身份域、完整搜索服务或 AuthZ 权限中心。
```

不同场景要裁剪：

```text
面试：少图、强解释、能追问；
技术分享：按主线逐步展开；
README：只放定位图和核心架构图；
答辩：图要能回链证据；
复盘：可以补运行时、Suggest 和护栏图。
```

---

## 3. 图谱总览

推荐把 IAM 架构图分成九类：

```text
A. 项目定位图
B. 系统架构图
C. 运行时装配图
D. AuthN 认证链路图
E. AuthZ 授权链路图
F. Identity / ProfileLink 图
G. IDP 与第三方登录图
H. 接入与工程护栏图
I. Suggest 辅助读模型图
```

| 类别 | 图名 | 用途 | 优先级 |
| --- | --- | --- | --- |
| A | IAM 总定位图 | 讲 IAM 不是普通用户中心，并说明 Suggest 是辅助读模型 | P0 |
| A | 普通用户中心 vs IAM | 讲系统定位差异 | P1 |
| B | 分层架构主图 | 讲 Transport/Application/Domain/Infra | P0 |
| B | 横向模块边界图 | 讲 AuthN/AuthZ/Identity/IDP + Suggest 辅助读模型 | P0 |
| B | 六边形架构图 | 讲 driving/driven adapters，包含 Suggest Search adapter | P1 |
| C | 服务启动与组合根图 | 讲 process/container/REST/gRPC/runtime task/Suggest refresh | P1 |
| C | Container 能力投影图 | 讲 composition root，不是 service locator | P1 |
| D | AuthN 模型关系图 | 讲 User/LoginIdentity/Credential/Challenge | P0 |
| D | AuthN 登录主链路图 | 讲 Login -> Principal -> Session -> Token | P0 |
| D | Session/Access/Refresh 分工图 | 讲登录态和凭证边界 | P0 |
| D | JWKS vs 在线 Verify 图 | 讲签名可信 vs 当前可用 | P0 |
| D | KeyRotation 状态机 | 讲 active/grace/retired | P1 |
| E | AuthZ 授权模型图 | 讲 Subject/RoleBinding/Role/Permission/Resource | P0 |
| E | AuthZ Check 链路图 | 讲 AuthorizationRequest -> DecisionEngine -> Casbin | P0 |
| E | AuthZ 写入链路图 | 讲 PolicyChange/UoW/Version/Outbox/Reload | P0 |
| E | Outbox 授权版本传播图 | 讲版本变化如何可靠传播 | P1 |
| E | Outbox 状态机 | 讲 at-least-once 与重试 | P1 |
| F | User/Profile/ProfileLink 图 | 讲登录主体与业务档案关系 | P0 |
| F | 当前用户 Profile 访问 guard 图 | 讲 MyProfiles 防越权 | P1 |
| F | Identity 与 AuthN/AuthZ/Suggest 边界图 | 讲 ProfileLink 不是 Permission，也不是 ProfileAccessScope | P1 |
| F | ProfileLink 与 Suggest 可见性边界图 | 讲 ProfileLink 可参与可见性读模型，但不等于 ProfileAccessScope | P1 |
| G | IDP 与 AuthN 边界图 | 讲 IDP 只做身份源基础设施 | P0 |
| G | 微信登录链路图 | 讲 code2Session 不等于 IAM 登录成功 | P1 |
| G | 双 access token 区分图 | 讲平台 token 与 IAM token 区别 | P1 |
| H | REST/gRPC/SDK 接入模型图 | 讲三类调用方和三种接入方式，包含 Suggest REST | P0 |
| H | 接入方式选择图 | 讲不同场景如何选 REST/gRPC/SDK/Suggest REST | P1 |
| H | qs-server 接入 IAM 主链路图 | 讲业务系统如何接 IAM | P0 |
| H | 工程护栏总图 | 讲 architecture/contract/SDK/docs/Suggest 防漂移 | P1 |
| H | 分层依赖护栏图 | 讲 architecture tests 保护什么 | P1 |
| H | 契约防漂移图 | 讲 REST/gRPC/SDK/Suggest 如何防漂移 | P1 |
| I | Suggest 模块定位图 | 讲 Suggest 是辅助读模型，不是核心身份域 | P0 |
| I | Suggest 查询链路图 | 讲 OperatingPrincipal -> ProfileAccessScope -> Trie/Hash -> mobile_mask | P0 |
| I | Suggest 索引模型图 | 讲 ProfileSearchTerm / Trie / Hash / Runtime | P1 |
| I | Suggest 刷新链路图 | 讲 Loader / Full / Delta / Runtime Replace / ImportDelta | P1 |
| I | Suggest 安全护栏图 | 讲手机号搜索、限流、指标、降级 | P1 |

---

## A. 项目定位图

### A1. IAM 总定位图

#### 适用文档

```text
00-项目一句话定位.md
01-业务背景与问题.md
02-系统架构讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
这个项目到底是什么？
为什么不是普通用户中心？
IAM 包含哪些核心能力？
Suggest 在 IAM 里应该怎么定位？
```

#### Mermaid

```mermaid
flowchart TD
    IAM["IAM<br/>身份与访问管理服务"]

    AuthN["AuthN<br/>登录 / Session / Token / JWKS"]
    AuthZ["AuthZ<br/>Role / Resource / Permission / Check"]
    Identity["Identity<br/>User / Profile / ProfileLink"]
    IDP["IDP<br/>Provider App / SecretVault / Provider API"]
    Access["Access<br/>REST / gRPC / SDK"]
    Suggest["Suggest<br/>Profile联想搜索辅助读模型"]
    Guard["Guard<br/>架构测试 / 契约测试 / docs-hygiene"]

    IAM --> AuthN
    IAM --> AuthZ
    IAM --> Identity
    IAM --> IDP
    IAM --> Access
    IAM -.辅助能力.-> Suggest
    IAM --> Guard
```

#### 讲图台词

```text
我会先把 IAM 拆成主线能力和辅助能力：AuthN 管认证态，AuthZ 管访问权，Identity 管身份与档案关系，IDP 管第三方身份源，Access 管业务系统接入，Guard 管长期演进不漂移。Suggest 是内置辅助读模型，服务后台 Profile autocomplete，不是 IAM 核心身份域。
```

---

### A2. 普通用户中心 vs IAM

#### 适用文档

```text
00-项目一句话定位.md
01-业务背景与问题.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
为什么不能说这是用户中心？
普通 users 表 CRUD 和 IAM 的差别在哪里？
为什么管理后台还需要 Suggest？
```

#### Mermaid

```mermaid
flowchart TB
    subgraph UserCenter["普通用户中心"]
        UCUser["User"]
        UCCRUD["User CRUD"]
        UCLogin["Simple Login"]
        UCJWT["JWT"]
        UCSearch["LIKE 查询"]
        UCUser --> UCCRUD
        UCLogin --> UCJWT
        UCCRUD --> UCSearch
    end

    subgraph IAM["IAM"]
        AuthN["AuthN<br/>Login / Session / Token"]
        AuthZ["AuthZ<br/>Role / Permission / Check"]
        Identity["Identity<br/>User / Profile / ProfileLink"]
        IDP["IDP<br/>External Identity Source"]
        Suggest["Suggest<br/>Profile autocomplete read model"]
        Access["REST / gRPC / SDK"]
        Outbox["PolicyVersion / Outbox"]
        Guard["Architecture Guard"]

        AuthN --> Identity
        AuthZ --> Identity
        AuthN --> IDP
        Identity -.Profile投影.-> Suggest
        AuthZ -.ProfileAccessScope.-> Suggest
        AuthZ --> Outbox
        Access --> AuthN
        Access --> AuthZ
        Access --> Identity
        Access --> Suggest
        Guard --> Access
    end
```

#### 讲图台词

```text
普通用户中心的核心是 User CRUD 和简单登录，而 IAM 的核心是身份、认证、授权、会话、凭证、第三方身份源和业务系统接入。后台 Profile autocomplete 也不是简单 LIKE 查询，而是通过 Suggest 读模型处理权限过滤、手机号安全和低延迟查询。
```

---

## B. 系统架构图

### B1. 分层架构主图

#### 适用文档

```text
02-系统架构讲法.md
10-工程质量与架构护栏讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
项目整体架构是什么？
你怎么组织代码层次？
六边形架构体现在哪里？
Suggest 如何纳入分层而不破坏边界？
```

#### Mermaid

```mermaid
flowchart TD
    Clients["External Clients<br/>Web / App / Backend / SDK / Admin"]
    Transport["Transport<br/>REST handlers / gRPC services / middleware"]
    Application["Application<br/>Use cases / UoW / orchestration"]
    Domain["Domain<br/>AuthN / AuthZ / Identity / Suggest rules"]
    Infra["Infrastructure<br/>MySQL / Redis / Casbin / JWT / Outbox / IDP API / Suggest Search"]

    Clients --> Transport
    Transport --> Application
    Application --> Domain
    Application --> Infra
    Infra --> Domain
```

#### 讲图台词

```text
Transport 只做协议适配，Application 负责编排用例和事务，Domain 承载业务规则，Infra 适配外部资源。核心逻辑不依赖 REST、gRPC、MySQL、Redis、Casbin 或 Suggest Trie/Hash 的具体实现。
```

---

### B2. 横向模块边界图

#### 适用文档

```text
02-系统架构讲法.md
03-AuthN认证体系讲法.md
04-AuthZ授权体系讲法.md
05-Identity与ProfileLink讲法.md
06-IDP与第三方登录讲法.md
09-REST-gRPC-SDK接入讲法.md
```

#### 适合回答

```text
为什么拆 AuthN/AuthZ/Identity/IDP？
模块之间怎么协作？
Suggest 是不是和这些模块同级？
```

#### Mermaid

```mermaid
flowchart LR
    AuthN["AuthN<br/>Login / Session / Token / JWKS"]
    AuthZ["AuthZ<br/>Role / Resource / Permission / Check"]
    Identity["Identity<br/>User / Profile / ProfileLink"]
    IDP["IDP<br/>Provider App / SecretVault / Provider API"]
    Suggest["Suggest<br/>Profile autocomplete read model"]
    Access["REST / gRPC / SDK"]
    Outbox["Transactional Outbox"]

    Access --> AuthN
    Access --> AuthZ
    Access --> Identity
    Access --> IDP
    Access --> Suggest

    AuthN -->|"User status / principal anchor"| Identity
    AuthN -->|"external identity source"| IDP
    AuthZ -->|"subject=user:<id>"| Identity
    AuthZ -->|"version_changed"| Outbox
    Identity -."ProfileSearchTerm projection".-> Suggest
    AuthZ -."ProfileAccessScope context".-> Suggest
```

#### 讲图台词

```text
AuthN、AuthZ、Identity、IDP 是四类主线问题：认证态、访问权、身份关系和外部身份源。Suggest 是辅助读模型，使用 ProfileSearchTerm 和 ProfileAccessScope 服务后台 autocomplete，不是 IAM 核心身份域，也不是 AuthZ 权限中心。
```

---

### B3. 六边形架构图

#### 适用文档

```text
02-系统架构讲法.md
10-工程质量与架构护栏讲法.md
```

#### 适合回答

```text
你说用了六边形架构，哪里体现？
REST/gRPC、MySQL、Redis、Casbin、Suggest Search 分别是什么角色？
```

#### Mermaid

```mermaid
flowchart LR
    subgraph Core["Core"]
        Domain["Domain<br/>Entities / Value Objects / Domain Services / Policies"]
        Application["Application<br/>Use Cases / Ports / UoW"]
        Application --> Domain
    end

    subgraph Driving["Driving Adapters"]
        REST["REST"]
        SuggestREST["Suggest REST"]
        GRPC["gRPC"]
        SDKConsumer["SDK Consumer"]
    end

    subgraph Driven["Driven Adapters"]
        MySQL["MySQL Repository"]
        Redis["Redis Store"]
        Casbin["Casbin Adapter"]
        JWT["JWT / JWKS"]
        IDPAPI["IDP Provider API"]
        Outbox["Outbox Store"]
        SuggestSearch["Suggest Search<br/>Trie / Hash / Runtime"]
    end

    REST --> Application
    SuggestREST --> Application
    GRPC --> Application
    SDKConsumer --> REST
    SDKConsumer --> GRPC

    Application --> MySQL
    Application --> Redis
    Application --> Casbin
    Application --> JWT
    Application --> IDPAPI
    Application --> Outbox
    Application --> SuggestSearch

    MySQL --> Domain
    Casbin --> Domain
    SuggestSearch --> Domain
```

#### 讲图台词

```text
REST/gRPC/Suggest REST 是驱动适配器，MySQL/Redis/Casbin/JWT/IDP API/Outbox/Suggest Search 是被驱动适配器，中间的 Application/Domain 才是核心。
```

---

## C. 运行时装配图

### C1. 服务启动与组合根图

#### 适用文档

```text
02-系统架构讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
服务如何启动？
process 和 container 分别负责什么？
container 是不是 Service Locator？
Suggest refresh 如何纳入生命周期？
```

#### Mermaid

```mermaid
flowchart TD
    Main["cmd/apiserver"]
    App["NewApp().Run"]
    Process["process.Run"]
    Prepare["PrepareRun"]
    Container["container.Initialize"]
    Modules["AuthN / AuthZ / Identity / IDP / Suggest"]
    REST["BuildRESTDeps -> REST Router"]
    GRPC["BuildGRPCDeps -> gRPC Registry"]
    Runtime["Runtime Tasks<br/>Outbox Relay / Scheduler / Suggest Refresh"]
    Shutdown["Graceful Shutdown"]

    Main --> App --> Process --> Prepare --> Container --> Modules
    Container --> REST
    Container --> GRPC
    Container --> Runtime
    Runtime --> Shutdown
```

#### 讲图台词

```text
main 很薄，process 负责生命周期，container 是组合根，模块能力最终投影给 REST、gRPC 和 runtime tasks。Suggest 的 Runtime、Full/Delta refresh、RateLimiter、Metrics、DegradedService 也在启动阶段装配并纳入生命周期。
```

---

### C2. Container 能力投影图

#### 适用文档

```text
02-系统架构讲法.md
10-工程质量与架构护栏讲法.md
```

#### 适合回答

```text
container 是不是 Service Locator？
REST/gRPC 怎么拿依赖？
Suggest runtime 怎么被装配？
```

#### Mermaid

```mermaid
flowchart TD
    Container["Container<br/>Composition Root"]
    AuthN["AuthN Module"]
    AuthZ["AuthZ Module"]
    Identity["Identity Module"]
    IDP["IDP Module"]
    Suggest["Suggest Module"]

    RestDeps["REST Deps"]
    GrpcRegs["gRPC Registrations"]
    RuntimeDeps["Runtime Deps"]

    Container --> AuthN
    Container --> AuthZ
    Container --> Identity
    Container --> IDP
    Container --> Suggest

    Container --> RestDeps
    Container --> GrpcRegs
    Container --> RuntimeDeps

    RestDeps --> REST["REST Router"]
    GrpcRegs --> GRPC["gRPC Registry"]
    RuntimeDeps --> Tasks["Runtime Tasks"]
```

#### 讲图台词

```text
container 只在启动阶段负责组装和能力投影，业务请求不会到 container 里随便取对象，所以它是 composition root，不是运行期 Service Locator。Suggest 也通过 container 装配成明确的 application、runtime 和 REST deps。
```

---

## D. AuthN 认证链路图

### D1. AuthN 模型关系图

#### 适用文档

```text
03-AuthN认证体系讲法.md
```

#### 适合回答

```text
User、LoginIdentity、Credential、Challenge 是什么关系？
为什么登录方式不直接塞进 User？
```

#### Mermaid

```mermaid
flowchart LR
    User["User<br/>Identity 主体"]
    LoginIdentity["LoginIdentity<br/>登录身份"]
    Credential["Credential<br/>稳定认证材料"]
    Challenge["Challenge<br/>临时认证挑战"]
    Principal["Principal<br/>认证主体摘要"]
    Session["Session<br/>登录态锚点"]
    AT["AccessToken<br/>短期访问凭证"]
    RT["RefreshToken<br/>续期凭证"]

    User --> LoginIdentity
    LoginIdentity --> Credential
    LoginIdentity --> Challenge
    LoginIdentity --> Principal
    Principal --> Session
    Session --> AT
    Session --> RT
```

#### 讲图台词

```text
User 是 Identity 的主体，LoginIdentity 是 AuthN 的登录入口，Credential 和 Challenge 是证明材料，认证成功后产出 Principal，再由 Session 和 Token 链路管理登录态。
```

---

### D2. AuthN 登录主链路图

#### 适用文档

```text
03-AuthN认证体系讲法.md
07-JWKS与Token安全讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
登录流程怎么设计？
为什么不是直接发 JWT？
不同登录方式如何统一？
```

#### Mermaid

```mermaid
sequenceDiagram
    participant Client as Client
    participant Login as Login Application
    participant Method as Login Method / Adapter
    participant Authenticator as Domain Authenticator
    participant Issuer as Token Issuer
    participant Session as Session Store
    participant Refresh as RefreshToken Store

    Client->>Login: Login(method, payload)
    Login->>Method: build proof
    Method-->>Login: auth proof
    Login->>Authenticator: authenticate(proof)
    Authenticator-->>Login: Principal
    Login->>Issuer: issue token pair(principal)
    Issuer->>Session: create session
    Issuer->>Refresh: save refresh token(session_id)
    Issuer-->>Client: AccessToken + RefreshToken
```

#### 讲图台词

```text
不同登录方式先被适配成认证 proof，领域认证器输出 Principal，Token Issuer 再创建 Session 并签发 AccessToken / RefreshToken。
```

---

### D3. Session / Access / Refresh 分工图

#### 适用文档

```text
03-AuthN认证体系讲法.md
07-JWKS与Token安全讲法.md
```

#### 适合回答

```text
AccessToken 和 RefreshToken 怎么分工？
为什么需要 Session？
为什么不能只用 JWT？
```

#### Mermaid

```mermaid
flowchart TD
    Principal["Principal"]
    Session["Session<br/>online login anchor"]
    Access["AccessToken<br/>short-lived JWT"]
    Refresh["RefreshToken<br/>server-controlled credential"]

    Principal --> Session
    Session --> Access
    Session --> Refresh

    Access -->|"request auth / JWKS verify"| Business["Business APIs"]
    Refresh -->|"refresh token pair"| AuthN["AuthN Refresh"]
    AuthN --> Session
```

#### 讲图台词

```text
AccessToken 负责访问，RefreshToken 负责续期，Session 是二者共同绑定的在线登录态锚点。
```

---

### D4. JWKS vs 在线 Verify 图

#### 适用文档

```text
03-AuthN认证体系讲法.md
07-JWKS与Token安全讲法.md
```

#### 适合回答

```text
JWKS 和在线 Verify 有什么区别？
本地验签通过为什么不代表用户仍然有效？
```

#### Mermaid

```mermaid
flowchart TD
    Token["AccessToken JWT"]
    Local["JWKS Local Verify"]
    Online["Online Verify"]

    Crypto["Signature / kid / exp / aud / iss"]
    Revoked["Revoked Marker"]
    Session["Session Active"]
    Subject["User / Account Status"]

    Token --> Local --> Crypto
    Token --> Online --> Crypto
    Online --> Revoked
    Online --> Session
    Online --> Subject
```

#### 讲图台词

```text
JWKS 证明签名可信，在线 Verify 证明当前可用。两者不是替代关系。
```

---

### D5. KeyRotation 状态机

#### 适用文档

```text
07-JWKS与Token安全讲法.md
```

#### 适合回答

```text
JWKS 如何支持密钥轮换？
active / grace / retired 分别是什么？
```

#### Mermaid

```mermaid
stateDiagram-v2
    [*] --> active: new signing key
    active --> grace: rotate
    grace --> retired: grace period ended
    retired --> [*]

    note right of active
      sign new Token
      publish in JWKS
    end note

    note right of grace
      do not sign new Token
      still verify old Token
    end note

    note right of retired
      not used for normal verification
    end note
```

#### 讲图台词

```text
active 负责签新 Token，grace 负责验旧 Token，retired 退出正常验证路径。这个状态机让 KeyRotation 平滑过渡。
```

---

## E. AuthZ 授权链路图

### E1. AuthZ 授权模型图

#### 适用文档

```text
04-AuthZ授权体系讲法.md
08-Outbox与授权版本传播讲法.md
```

#### 适合回答

```text
授权模型怎么设计？
为什么不用 user.role？
RoleBinding 和 Permission 分别表达什么？
```

#### Mermaid

```mermaid
flowchart LR
    Subject["Subject<br/>user / service / group"]
    Tenant["TenantDomain"]
    RoleBinding["RoleBinding<br/>subject holds role in tenant"]
    Role["Role"]
    Permission["Permission<br/>resource + action + scope"]
    Resource["Resource"]
    Action["Action"]
    Scope["Scope"]
    Decision["AuthorizationDecision"]

    Subject --> RoleBinding
    Tenant --> RoleBinding
    RoleBinding --> Role
    Role --> Permission
    Permission --> Resource
    Permission --> Action
    Permission --> Scope
    Permission --> Decision
```

#### 讲图台词

```text
Subject 不是直接有权限，而是在某个 TenantDomain 下通过 RoleBinding 获得 Role；Role 再通过 Permission 关联 Resource、Action 和 Scope。这里的 TenantDomain 是授权域，不要和业务 org_id 混用。
```

---

### E2. AuthZ Check 链路图

#### 适用文档

```text
04-AuthZ授权体系讲法.md
```

#### 适合回答

```text
一次权限判定怎么走？
Casbin 在哪里？
业务服务要不要直接调用 Casbin？
```

#### Mermaid

```mermaid
sequenceDiagram
    participant Service as Business Service
    participant App as AuthZ Application
    participant Req as AuthorizationRequest
    participant Engine as DecisionEngine
    participant Casbin as CasbinAdapter

    Service->>App: Check(subject, tenantDomain, resource, action, scope)
    App->>Req: NewAuthorizationRequest
    App->>Engine: Check(request)
    Engine->>Casbin: Enforce(mapped facts)
    Casbin-->>Engine: allowed / denied
    Engine-->>App: AuthorizationDecision
    App-->>Service: AuthorizationDecision
```

#### 讲图台词

```text
业务服务不自己判断权限，也不直接调用 Casbin。它调用 IAM Check，AuthZ 应用层构造领域请求，DecisionEngine 负责判定，CasbinAdapter 只是 infra runtime 实现。
```

---

### E3. AuthZ 写入链路图

#### 适用文档

```text
04-AuthZ授权体系讲法.md
08-Outbox与授权版本传播讲法.md
```

#### 适合回答

```text
授权写入为什么不是 CRUD？
PolicyChangeCommitter 做什么？
```

#### Mermaid

```mermaid
flowchart TD
    Command["Grant / Revoke / Bind / Unbind"]
    Admin["PolicyAdministration"]
    Change["PolicyChange"]
    Committer["PolicyChangeCommitter"]
    UOW["AuthZ UoW Transaction"]
    Management["Management Records<br/>RoleBinding / Permission"]
    Facts["Runtime Facts<br/>Casbin p/g"]
    Version["PolicyVersion + 1"]
    Outbox["Outbox Event<br/>authz.version_changed"]
    Reload["RuntimeReload"]

    Command --> Admin --> Change --> Committer --> UOW
    UOW --> Management
    UOW --> Facts
    UOW --> Version
    UOW --> Outbox
    Committer --> Reload
```

#### 讲图台词

```text
授权写入不是单表 CRUD。PolicyChangeCommitter 在一个 UoW 中同时维护管理事实、运行时 facts、版本和 Outbox 事件，提交后再 reload runtime policy。
```

---

### E4. Outbox 授权版本传播图

#### 适用文档

```text
08-Outbox与授权版本传播讲法.md
04-AuthZ授权体系讲法.md
```

#### 适合回答

```text
为什么授权版本传播需要 Outbox？
DB commit 后直接 publish MQ 有什么问题？
```

#### Mermaid

```mermaid
sequenceDiagram
    participant App as PolicyChangeCommitter
    participant Tx as AuthZ UoW Tx
    participant Facts as Runtime Facts
    participant Version as PolicyVersion
    participant Outbox as domain_event_outbox
    participant Runtime as RuntimePolicyReload
    participant Relay as OutboxRelay
    participant Bus as EventBus
    participant Consumer as Business Service / SDK Cache

    App->>Tx: Commit policy change
    Tx->>Facts: write p/g facts
    Tx->>Version: increment tenant version
    Tx->>Outbox: stage version_changed event
    Tx-->>App: DB commit
    App->>Runtime: best-effort reload current process

    Relay->>Outbox: claim due events
    Relay->>Bus: publish version_changed
    Bus-->>Consumer: tenantDomain + version
    Relay->>Outbox: mark published / failed
```

#### 讲图台词

```text
授权 facts、PolicyVersion 和 outbox row 同事务提交；当前进程 reload 是提交后的本地刷新；事件发布是事务后的异步可靠投递。
```

---

### E5. Outbox 状态机

#### 适用文档

```text
08-Outbox与授权版本传播讲法.md
```

#### 适合回答

```text
Outbox 怎么重试？
为什么消费者要幂等？
Outbox 是 exactly-once 吗？
```

#### Mermaid

```mermaid
stateDiagram-v2
    [*] --> pending: stage in DB tx
    pending --> publishing: claim due event
    failed --> publishing: retry due
    publishing --> published: publish success + mark
    publishing --> failed: publish failed + retry
    publishing --> publishing: stale publishing re-claimed
```

#### 讲图台词

```text
Outbox 保证 at-least-once，不保证 exactly-once，所以消费者要按 event_id 或 tenantDomain + version 幂等。
```

---

## F. Identity / ProfileLink 图

### F1. User / Profile / ProfileLink 图

#### 适用文档

```text
05-Identity与ProfileLink讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
为什么 User 和 Profile 要分开？
ProfileLink 为什么不能只是 User 字段？
为什么不是 user.profile_id？
```

#### Mermaid

```mermaid
flowchart LR
    UserA["User A<br/>登录主体"]
    UserB["User B<br/>登录主体"]
    Link1["ProfileLink<br/>self"]
    Link2["ProfileLink<br/>parent"]
    Link3["ProfileLink<br/>grandparent"]
    ProfileA["Profile A<br/>本人档案"]
    ProfileB["Profile B<br/>儿童档案"]

    UserA --> Link1 --> ProfileA
    UserA --> Link2 --> ProfileB
    UserB --> Link3 --> ProfileB
```

#### 讲图台词

```text
User 是登录主体，Profile 是业务档案，ProfileLink 是二者之间的关系事实。它不是一对一字段，而是多对多关系边。
```

---

### F2. 当前用户 Profile 访问 guard 图

#### 适用文档

```text
05-Identity与ProfileLink讲法.md
```

#### 适合回答

```text
怎么防止用户访问别人的 Profile？
MyProfiles 为什么不能直接按 profile_id 查？
```

#### Mermaid

```mermaid
flowchart TD
    Req["GET / PATCH my profile"]
    CurrentUser["current user id"]
    ProfileID["profile id"]
    LinkCheck["Find active ProfileLink<br/>by user_id + profile_id"]
    Deny["deny"]
    LoadProfile["Load Profile"]
    Return["Return / Patch Profile"]

    Req --> CurrentUser
    Req --> ProfileID
    CurrentUser --> LinkCheck
    ProfileID --> LinkCheck
    LinkCheck -->|"not found"| Deny
    LinkCheck -->|"active link exists"| LoadProfile --> Return
```

#### 讲图台词

```text
当前用户不能直接按 profile_id 读写档案，必须先检查 current user 和 profile 之间是否存在 active ProfileLink。
```

---

### F3. Identity 与 AuthN/AuthZ/Suggest 边界图

#### 适用文档

```text
05-Identity与ProfileLink讲法.md
03-AuthN认证体系讲法.md
04-AuthZ授权体系讲法.md
09-REST-gRPC-SDK接入讲法.md
```

#### 适合回答

```text
Identity 和 AuthN/AuthZ/Suggest 怎么协作？
ProfileLink 是权限吗？
ProfileLink 是 Suggest 可见范围吗？
```

#### Mermaid

```mermaid
flowchart LR
    IDP["IDP<br/>external identity source"]
    AuthN["AuthN<br/>Login / Principal / Session / Token"]
    Identity["Identity<br/>User / Profile / ProfileLink"]
    AuthZ["AuthZ<br/>Subject / Resource / Action / Scope"]
    Suggest["Suggest<br/>Profile autocomplete"]
    ProfileSearchTerm["ProfileSearchTerm<br/>search read model"]
    ProfileAccessScope["ProfileAccessScope<br/>visible scope"]
    Business["Business System<br/>qs-server"]

    IDP -->|"external identity"| AuthN
    AuthN -->|"Principal.UserID"| Identity
    Identity -->|"User as subject"| AuthZ
    Business -->|"query Profile / ProfileLink"| Identity
    Business -->|"Check resource/action/scope"| AuthZ
    Identity -."ProfileLink is relation guard".-> Business
    Identity -."Profile projection".-> ProfileSearchTerm --> Suggest
    ProfileAccessScope --> Suggest
```

#### 讲图台词

```text
Identity 提供 User、Profile、ProfileLink。AuthN 使用 UserID 作为登录主体，AuthZ 使用 user:<id> 作为 subject，业务系统可以查询 ProfileLink 做身份关系判断，但最终资源访问仍应走 AuthZ Check。Suggest 可以使用 Profile 投影出的 ProfileSearchTerm 做联想搜索，但可见范围由 ProfileAccessScope 表达，不是直接由 ProfileLink 替代。
```

---

### F4. ProfileLink 与 Suggest 可见性边界图

#### 适用文档

```text
05-Identity与ProfileLink讲法.md
02-系统架构讲法.md
13-面试追问证据索引.md
```

#### 适合回答

```text
ProfileLink 和 ProfileAccessScope 有什么区别？
ProfileLink 变化如何影响 Suggest？
Suggest 为什么不每次直接查 ProfileLink？
```

#### Mermaid

```mermaid
flowchart TD
    Link["ProfileLink<br/>identity relation fact"]
    Visibility["Profile visibility read model"]
    ScopeProvider["ProfileAccessScopeProvider"]
    Scope["ProfileAccessScope"]
    Keyword["keyword"]
    Store["Suggest Store<br/>Trie / Hash"]
    Filter["scope filter"]
    Result["mobile_mask result"]

    Link -."may contribute".-> Visibility
    Visibility --> ScopeProvider --> Scope
    Keyword --> Store --> Filter --> Result
    Scope --> Filter
```

#### 讲图台词

```text
ProfileLink 可以是可见性读模型的事实来源之一，但 Suggest 查询时不应该直接扫描 ProfileLink。更好的边界是：ProfileLink 变化显式影响 visibility read model 或 ProfileAccessScopeProvider，然后 Suggest Store 只消费 ProfileAccessScope 做过滤。
```

---

## G. IDP 与第三方登录图

### G1. IDP 与 AuthN 边界图

#### 适用文档

```text
06-IDP与第三方登录讲法.md
03-AuthN认证体系讲法.md
```

#### 适合回答

```text
IDP 和 AuthN 的边界是什么？
为什么 IDP 不签 IAM token？
```

#### Mermaid

```mermaid
flowchart LR
    Client["Client<br/>wechat code / wecom auth_code"]
    AuthN["AuthN<br/>Login / Principal / Session / IAM Token"]
    IDP["IDP<br/>Provider App / SecretVault / External API"]
    Provider["WeChat / WeCom Platform"]
    Token["IAM TokenPair<br/>Access + Refresh"]

    Client --> AuthN
    AuthN -->|"query app config"| IDP
    AuthN -->|"decrypt secret"| IDP
    AuthN -->|"code exchange"| IDP
    IDP --> Provider
    AuthN -->|"binding + principal"| AuthN
    AuthN --> Token
```

#### 讲图台词

```text
IDP 只提供外部身份源能力，AuthN 仍然是登录编排者和 IAM Token 签发者。
```

---

### G2. 微信小程序登录链路图

#### 适用文档

```text
06-IDP与第三方登录讲法.md
03-AuthN认证体系讲法.md
```

#### 适合回答

```text
微信登录怎么接入 IAM？
code2Session 成功是不是登录成功？
```

#### Mermaid

```mermaid
sequenceDiagram
    participant Client as Mini Program
    participant AuthN as AuthN Login
    participant Method as WeChat LoginMethod
    participant Proof as ProofFactory
    participant IDP as IDP Repository / SecretVault
    participant WeChat as WeChat API
    participant Authenticator as Authenticator
    participant Token as Session / Token Issuer

    Client->>AuthN: login(app_id, js_code)
    AuthN->>Method: validate payload
    Method-->>AuthN: WeChat payload
    AuthN->>Proof: build auth proof
    Proof->>IDP: get app by app_id
    Proof->>IDP: decrypt AppSecret
    Proof->>WeChat: code2Session
    WeChat-->>Proof: openid / unionid
    Proof-->>AuthN: external identity proof
    AuthN->>Authenticator: authenticate / bind account
    Authenticator-->>AuthN: Principal
    AuthN->>Token: create Session + issue TokenPair
    Token-->>Client: IAM AccessToken + RefreshToken
```

#### 讲图台词

```text
code2Session 只能证明外部微信身份，不等于 IAM 登录成功。必须经过 AuthN 的账号绑定、Principal 和 Token 签发链路。
```

---

### G3. 双 access token 区分图

#### 适用文档

```text
06-IDP与第三方登录讲法.md
07-JWKS与Token安全讲法.md
```

#### 适合回答

```text
微信 access_token 和 IAM AccessToken 是什么关系？
为什么名字一样但不能混淆？
```

#### Mermaid

```mermaid
flowchart TB
    WechatToken["WeChat access_token<br/>调用微信平台 API"]
    IAMToken["IAM AccessToken<br/>访问 IAM / 业务系统"]
    IDP["IDP 管理"]
    AuthN["AuthN 管理"]
    WeChatAPI["WeChat / WeCom API"]
    BusinessAPI["Business APIs / IAM APIs"]

    IDP --> WechatToken --> WeChatAPI
    AuthN --> IAMToken --> BusinessAPI
```

#### 讲图台词

```text
微信 access_token 和 IAM AccessToken 名字像，但完全不是一回事。前者属于外部平台访问令牌，后者属于 IAM 登录态访问凭证。
```

---

## H. 接入与工程护栏图

### H1. REST / gRPC / SDK 接入模型图

#### 适用文档

```text
09-REST-gRPC-SDK接入讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
为什么要同时提供 REST、gRPC、SDK？
REST/gRPC/SDK 是不是三套业务逻辑？
Suggest REST 放在哪里？
```

#### Mermaid

```mermaid
flowchart TD
    Web["Web / App / Admin UI"]
    Service["Backend Service / Worker"]
    GoService["Go Business Service"]

    REST["REST API<br/>OpenAPI"]
    SuggestREST["Suggest REST<br/>/api/v2/suggest/profile"]
    GRPC["gRPC API<br/>Proto"]
    SDK["Go SDK<br/>Client Facade"]

    IAM["IAM Server<br/>AuthN / AuthZ / Identity / IDP / Suggest"]

    Web --> REST --> IAM
    Web --> SuggestREST --> IAM
    Service --> GRPC --> IAM
    GoService --> SDK
    SDK --> REST
    SDK --> GRPC
```

#### 讲图台词

```text
REST、gRPC、SDK 是同一套 IAM 能力面向不同调用方的接入投影，不是三套业务逻辑。Suggest REST 是 REST 接入的一部分，服务管理端 Profile autocomplete。
```

---

### H2. 接入方式选择图

#### 适用文档

```text
09-REST-gRPC-SDK接入讲法.md
```

#### 适合回答

```text
什么场景用 REST？
什么场景用 gRPC？
什么场景用 SDK？
Profile autocomplete 用什么？
```

#### Mermaid

```mermaid
flowchart LR
    Need["调用需求"]
    Login["用户登录 / 管理后台 / HTTP 调试"]
    SuggestNeed["Profile autocomplete"]
    S2S["服务间 Verify / AuthZ / Identity 查询"]
    GoApp["Go 服务接入 IAM"]

    REST["REST"]
    SuggestREST["Suggest REST"]
    GRPC["gRPC"]
    SDK["SDK"]

    Need --> Login --> REST
    Need --> SuggestNeed --> SuggestREST --> REST
    Need --> S2S --> GRPC
    Need --> GoApp --> SDK
```

#### 讲图台词

```text
选择接入方式不是看哪种技术更高级，而是看调用方和场景。Profile autocomplete 是管理端交互，优先走 Suggest REST。
```

---

### H3. qs-server 接入 IAM 主链路图

#### 适用文档

```text
09-REST-gRPC-SDK接入讲法.md
11-30分钟技术分享脚本.md
```

#### 适合回答

```text
业务系统如何接入 IAM？
qs-server 如何使用 Token、Identity 和 AuthZ？
```

#### Mermaid

```mermaid
sequenceDiagram
    participant Client as Client / Frontend
    participant IAMRest as IAM REST AuthN
    participant QS as qs-server
    participant SDK as IAM SDK / gRPC
    participant IAM as IAM Server

    Client->>IAMRest: Login
    IAMRest-->>Client: AccessToken + RefreshToken
    Client->>QS: Business API with Bearer Token
    QS->>SDK: VerifyToken
    SDK->>IAM: AuthN Verify
    IAM-->>SDK: Principal
    SDK-->>QS: Principal
    QS->>SDK: Identity Query if needed
    SDK->>IAM: GetUser / GetProfile / ListProfileLinks
    IAM-->>SDK: Identity data
    QS->>SDK: AuthZ Check(resource, action, scope)
    SDK->>IAM: Check
    IAM-->>SDK: AuthorizationDecision
    SDK-->>QS: allow / deny
    QS-->>Client: business response
```

#### 讲图台词

```text
前端登录 IAM，业务请求进 qs-server，qs-server 通过 SDK/gRPC 验 Token、查身份、做 AuthZ Check，不复制 IAM 的账号、角色和权限表。
```

---

### H4. 工程护栏总图

#### 适用文档

```text
10-工程质量与架构护栏讲法.md
06-架构护栏/README.md
```

#### 适合回答

```text
你怎么保证系统长期不乱？
工程质量怎么体现？
Suggest 如何防止越界和漂移？
```

#### Mermaid

```mermaid
flowchart TD
    Quality["工程质量与架构护栏"]

    Arch["架构测试<br/>依赖方向 / 分层边界"]
    REST["REST 契约<br/>路由 / OpenAPI"]
    GRPC["gRPC 契约<br/>proto / registration"]
    SDK["SDK 公开面<br/>compile test"]
    SuggestGuard["Suggest 护栏<br/>ProfileAccessScope / mobile_mask / 降级"]
    Docs["文档事实源<br/>docs-hygiene"]

    Quality --> Arch
    Quality --> REST
    Quality --> GRPC
    Quality --> SDK
    Quality --> SuggestGuard
    Quality --> Docs
```

#### 讲图台词

```text
工程质量不是只靠业务测试，而是把架构边界、接口契约、SDK 公开面、Suggest 安全边界和文档事实源都做成自动化或半自动化护栏。
```

---

### H5. 分层依赖护栏图

#### 适用文档

```text
10-工程质量与架构护栏讲法.md
06-架构护栏/01-架构测试与依赖边界.md
```

#### 适合回答

```text
architecture tests 保护什么？
为什么说不是目录洁癖？
Suggest 是否也受分层约束？
```

#### Mermaid

```mermaid
flowchart TD
    Transport["Transport<br/>REST / gRPC"]
    Application["Application<br/>Use Case / UoW"]
    Domain["Domain<br/>Business Rules"]
    Infra["Infra<br/>MySQL / Redis / Casbin / JWT / Outbox / Suggest Search"]
    Container["Container<br/>Composition Root"]
    SDK["pkg/sdk<br/>External Go API"]

    Transport --> Application
    Application --> Domain
    Infra --> Domain
    Container --> Application
    Container --> Infra

    Domain -. forbidden .-> Infra
    Domain -. forbidden .-> Transport
    Application -. forbidden .-> Transport
    Application -. forbidden .-> Infra
    Transport -. forbidden .-> Container
    Transport -. forbidden .-> Infra
    SDK -. forbidden .-> Internal["internal/apiserver"]
```

#### 讲图台词

```text
架构测试保护的是依赖方向，不是目录洁癖。核心是业务规则向内，协议和基础设施向外。Suggest 也遵守这套分层：REST handler 不直接操作索引，domain 不依赖 infra/search。
```

---

### H6. 契约防漂移图

#### 适用文档

```text
10-工程质量与架构护栏讲法.md
06-架构护栏/02-文档事实源与防漂移机制.md
09-REST-gRPC-SDK接入讲法.md
```

#### 适合回答

```text
如何防止 REST/gRPC/SDK 文档和代码不一致？
契约事实源是什么？
Suggest REST 如何防漂移？
```

#### Mermaid

```mermaid
flowchart LR
    RESTCode["Runtime REST Routes"]
    OpenAPI["api/rest OpenAPI"]
    RESTTest["router matrix / route contract"]

    SuggestYAML["api/rest/suggest.v2.yaml"]
    SuggestHandler["transport/rest/suggest"]
    SuggestTests["Suggest tests"]
    SuggestDocs["docs/08-Suggest"]

    Proto["api/grpc proto"]
    GRPCReg["gRPC service registration"]
    ProtoTest["proto contract test"]

    SDKAPI["pkg/sdk public API"]
    Compile["public API compile test"]

    RESTCode <--> RESTTest <--> OpenAPI
    SuggestYAML <--> SuggestTests <--> SuggestHandler
    SuggestTests <--> SuggestDocs
    Proto <--> ProtoTest <--> GRPCReg
    SDKAPI --> Compile
```

#### 讲图台词

```text
REST、gRPC、SDK 各自有事实源和契约测试。Suggest REST 额外要求 api/rest/suggest.v2.yaml、transport/rest/suggest、docs/08-Suggest 和相关测试保持一致，防止 mobile_mask、ProfileAccessScope、手机号搜索权限等安全语义漂移。
```

---

## I. Suggest 辅助读模型图

### I1. Suggest 模块定位图

#### 适用文档

```text
02-系统架构讲法.md
05-Identity与ProfileLink讲法.md
09-REST-gRPC-SDK接入讲法.md
10-工程质量与架构护栏讲法.md
11-30分钟技术分享脚本.md
13-面试追问证据索引.md
```

#### 适合回答

```text
Suggest 为什么在 IAM 里？
Suggest 是不是核心身份域？
Suggest 和 Identity/AuthZ 的关系是什么？
```

#### Mermaid

```mermaid
flowchart TD
    IAM["iam-apiserver"]
    Mainline["主线能力<br/>AuthN / AuthZ / Identity / IDP"]
    Suggest["Suggest<br/>Profile autocomplete read model"]
    Admin["Admin / Operating UI"]
    Identity["Identity<br/>Profile / ProfileLink"]
    AuthZ["AuthZ / Visibility<br/>ProfileAccessScope"]

    IAM --> Mainline
    IAM -.内置辅助读模型.-> Suggest
    Admin -->|"GET /api/v2/suggest/profile"| Suggest
    Identity -."Profile projection".-> Suggest
    AuthZ -."visible scope".-> Suggest
```

#### 讲图台词

```text
Suggest 暂时内置在 iam-apiserver 内，是 Profile autocomplete 辅助读模型。它使用 Profile 投影数据和 ProfileAccessScope 做过滤，但不是 IAM 核心身份域，也不是完整搜索服务或 AuthZ 权限中心。
```

---

### I2. Suggest 查询链路图

#### 适用文档

```text
02-系统架构讲法.md
09-REST-gRPC-SDK接入讲法.md
13-面试追问证据索引.md
```

#### 适合回答

```text
一次 Suggest 请求怎么走？
如何避免返回不该看的 Profile？
手机号为什么返回 mobile_mask？
```

#### Mermaid

```mermaid
flowchart TD
    Admin["Admin / Operating UI"]
    REST["GET /api/v2/suggest/profile"]
    Handler["Suggest REST Handler"]
    Principal["OperatingPrincipal"]
    Service["SuggestProfile Service"]
    ScopeProvider["ProfileAccessScopeProvider"]
    Scope["ProfileAccessScope"]
    Runtime["ProfileSuggestionRuntime"]
    Store["Trie / Hash Store"]
    Filter["ScopePolicy Filter"]
    Rank["RankingPolicy"]
    DTO["mobile_mask DTO"]

    Admin --> REST --> Handler --> Principal --> Service
    Service --> ScopeProvider --> Scope
    Service --> Runtime --> Store
    Store --> Filter --> Rank --> DTO
    Scope --> Filter
```

#### 讲图台词

```text
请求进入 Suggest REST 后先得到 OperatingPrincipal，再解析 ProfileAccessScope，然后从 Runtime 当前索引中用 Trie/Hash 召回候选，经过 scope filter 和 ranking 后返回 mobile_mask DTO。这里的关键是先过滤可见范围，再返回候选。
```

---

### I3. Suggest 索引模型图

#### 适用文档

```text
02-系统架构讲法.md
13-面试追问证据索引.md
```

#### 适合回答

```text
为什么不用直接 MySQL LIKE？
Trie / Hash / Runtime 分别做什么？
ProfileSearchTerm 是什么？
```

#### Mermaid

```mermaid
flowchart TD
    Term["ProfileSearchTerm<br/>ProfileID / Name / MobileMask / OrgID / OperatorIDs"]
    Tokenizer["Term Builder<br/>中文名 / 拼音 / 简拼 / ProfileID / mobile"]
    Trie["Trie / TST<br/>prefix match"]
    Hash["Hash Index<br/>ProfileID / mobile exact match"]
    Runtime["ProfileSuggestionRuntime<br/>current active store"]
    Query["Query keyword"]
    Candidates["Candidates"]

    Term --> Tokenizer
    Tokenizer --> Trie
    Tokenizer --> Hash
    Trie --> Runtime
    Hash --> Runtime
    Query --> Runtime --> Candidates
```

#### 讲图台词

```text
ProfileSearchTerm 是为搜索构建的读模型投影，不是 Profile 聚合本体。Trie/TST 负责姓名、拼音、简拼前缀匹配，Hash 负责 ProfileID 和手机号精确匹配，Runtime 持有当前活动索引。
```

---

### I4. Suggest 刷新链路图

#### 适用文档

```text
02-系统架构讲法.md
10-工程质量与架构护栏讲法.md
13-面试追问证据索引.md
```

#### 适合回答

```text
Suggest 索引如何更新？
Full refresh 和 Delta refresh 有什么区别？
旧姓名、旧手机号怎么避免残留？
```

#### Mermaid

```mermaid
flowchart TD
    MySQL["MySQL<br/>Profile / visibility source"]
    Loader["ProfileSearchTerm Loader"]
    Full["Full Refresh<br/>build new store"]
    Delta["Delta Refresh<br/>changed terms / tombstones"]
    Snapshot["Snapshot"]
    Runtime["ProfileSuggestionRuntime"]
    Replace["Runtime.Replace"]
    Import["Runtime.ImportDelta"]
    Active["Active Store"]

    MySQL --> Loader
    Loader --> Full --> Snapshot --> Replace --> Runtime --> Active
    Loader --> Delta --> Import --> Runtime
```

#### 讲图台词

```text
Full refresh 构建一个新的完整 store，再通过 Runtime.Replace 原子替换当前活动索引；Delta refresh 导入变化 term 或 tombstone，修正增量变化。旧 key 残留要靠完整的 Delta tombstone 协议和 Full refresh 兜底。
```

---

### I5. Suggest 安全护栏图

#### 适用文档

```text
10-工程质量与架构护栏讲法.md
13-面试追问证据索引.md
```

#### 适合回答

```text
手机号搜索为什么敏感？
Suggest 挂了怎么办？
如何观测和限流？
```

#### Mermaid

```mermaid
flowchart TD
    Keyword["keyword"]
    MobileShape["is mobile-like?"]
    Allow["AllowMobileSearch?"]
    RateLimit["RateLimiter"]
    Scope["ProfileAccessScope"]
    Match["Trie / Hash Match"]
    Mask["mobile_mask"]
    Metrics["Metrics"]
    Degraded["DegradedService<br/>return []"]

    Keyword --> RateLimit --> MobileShape
    MobileShape -->|"yes"| Allow
    MobileShape -->|"no"| Match
    Allow -->|"no"| Degraded
    Allow -->|"yes"| Match
    Scope --> Match
    Match --> Mask
    Match --> Metrics
    Degraded --> Metrics
```

#### 讲图台词

```text
手机号形态关键词是高敏感能力，需要额外授权和限流。无权限或降级时返回空数组，比暴露拒绝细节或绕过权限直接查库更安全。响应只返回 mobile_mask，不返回明文 mobile。
```

---

## J. 面试推荐核心图

如果是面试，建议优先准备 5 张主线图：

```text
1. IAM 总定位图；
2. 分层架构主图；
3. AuthN 登录主链路图；
4. AuthZ 授权模型图 + AuthZ 写入链路图；
5. User / Profile / ProfileLink 图。
```

原因：

| 图 | 面试价值 |
| --- | --- |
| IAM 总定位图 | 迅速说明项目不是普通用户中心 |
| 分层架构主图 | 展示工程架构能力 |
| AuthN 登录主链路图 | 展示认证与 Token 安全理解 |
| AuthZ 授权模型 + 写入链路图 | 展示复杂业务建模与一致性治理 |
| User/Profile/ProfileLink 图 | 展示领域建模能力 |

如果还有时间，准备第 6 张：

```text
6. Suggest 查询链路图。
```

它适合回答：

```text
为什么不直接 MySQL 查询；
如何做 ProfileAccessScope 过滤；
为什么返回 mobile_mask；
为什么 Suggest 是辅助读模型而不是 AuthZ 中心。
```

如果还有时间，再补：

```text
JWKS vs 在线 Verify；
Outbox 状态机；
REST/gRPC/SDK 接入模型；
Suggest 刷新链路图；
工程护栏总图。
```

面试白板时不要追求图复杂。

每张图只讲一个问题：

```text
定位图：这个项目是什么；
分层图：项目如何组织；
AuthN 图：登录如何变成 Token；
AuthZ 图：权限如何判定和传播；
ProfileLink 图：身份关系如何建模；
Suggest 图：后台联想搜索如何兼顾性能和权限边界。
```

---

## K. 30 分钟分享推荐图序

如果是 30 分钟技术分享，推荐图序：

```text
1. IAM 总定位图；
2. 分层架构主图；
3. 横向模块边界图；
4. AuthN 登录主链路图；
5. JWKS vs 在线 Verify 图；
6. AuthZ 授权模型图；
7. AuthZ 写入链路图；
8. User / Profile / ProfileLink 图；
9. IDP 与 AuthN 边界图；
10. REST / gRPC / SDK 接入模型图；
11. Suggest 查询链路图；
12. 工程护栏总图。
```

时间不足时删掉：

```text
KeyRotation 状态机；
Outbox 状态机；
Container 能力投影图；
当前用户 Profile guard 图；
契约防漂移图；
Suggest 索引模型图；
Suggest 刷新链路图；
Suggest 安全护栏图。
```

不要删：

```text
定位图；
分层图；
AuthN 图；
AuthZ 图；
ProfileLink 图。
```

如果分享对象对工程细节感兴趣，再保留：

```text
Suggest 查询链路图；
工程护栏总图。
```

---

## L. 图的使用规则

### L1. 一张图只讲一个问题

错误：

```text
一张图塞进所有模块、所有表、所有接口、所有依赖。
```

正确：

```text
每张图只回答一个核心问题。
```

例如：

```text
AuthN 登录主链路图 -> 登录如何变成 Token；
JWKS vs Verify 图 -> 签名可信和当前可用的区别；
ProfileLink 图 -> User/Profile 为什么不是一对一；
Suggest 查询链路图 -> autocomplete 如何做权限过滤；
Outbox 状态机 -> 为什么消费者要幂等。
```

---

### L2. 面试时少画细节，多画边界

面试官通常更关心：

```text
你怎么拆边界？
你为什么这么设计？
失败时怎么办？
这套设计如何演进？
```

不一定关心：

```text
每个 handler 名字；
每个 DTO 字段；
每个表字段；
每个测试函数名。
```

---

### L3. 不要画出你讲不清楚的东西

如果一张图里有：

```text
你没看过的源码；
你不理解的模块；
你无法解释的箭头；
你无法回链证据的未来能力。
```

不要放进面试图里。

---

### L4. 图要能回链证据

每张图都应该能回链到：

```text
源码；
OpenAPI；
proto；
SDK；
测试；
事实层文档。
```

Suggest 图应该能回链到：

```text
docs/08-Suggest；
api/rest/suggest.v2.yaml；
internal/apiserver/transport/rest/suggest；
internal/apiserver/application/suggest；
internal/apiserver/domain/suggest；
internal/apiserver/infra/suggest；
internal/apiserver/infra/mysql/suggest。
```

不要画“凭感觉”的图。

---

### L5. 不要把规划能力画成当前能力

比如：

```text
完整多租户管理平台；
完整 OAuth/OIDC Provider 平台；
完整服务间 mTLS；
完整多语言 SDK；
完整可视化权限运维平台；
独立 Suggest 搜索服务；
完整搜索平台。
```

如果当前代码尚未完整实现，就不要画成已完成能力。

可以写成：

```text
后续演进方向；
架构预留；
暂未完整实现。
```

---

### L6. Suggest 图不要误导边界

不要画成：

```text
Suggest = AuthZ；
Suggest = Identity 核心域；
ProfileLink = ProfileAccessScope；
Suggest 直接返回明文 mobile；
Suggest 每次查询直接扫描 ProfileLink；
Suggest 索引不可用时直接查 MySQL 返回全局结果。
```

推荐画成：

```text
OperatingPrincipal
  -> ProfileAccessScope
  -> Trie / Hash match
  -> scope filter
  -> mobile_mask result
```

---

## M. 证据链索引

| 图类 | 事实层证据 |
| --- | --- |
| 项目定位图 | `docs/00-概览`、`docs/07-宣讲/00-项目一句话定位.md` |
| 业务背景图 | `docs/07-宣讲/01-业务背景与问题.md` |
| 系统架构图 | `docs/00-概览`、`docs/01-运行时`、`docs/07-宣讲/02-系统架构讲法.md` |
| AuthN 图 | `docs/02-认证AuthN`、`docs/07-宣讲/03-AuthN认证体系讲法.md` |
| Token / JWKS 图 | `docs/02-认证AuthN/04-*`、`docs/02-认证AuthN/05-*`、`docs/02-认证AuthN/06-*`、`docs/07-宣讲/07-JWKS与Token安全讲法.md` |
| AuthZ 图 | `docs/03-授权AuthZ`、`docs/07-宣讲/04-AuthZ授权体系讲法.md` |
| Outbox 图 | `docs/03-授权AuthZ/04-*`、`docs/07-宣讲/08-Outbox与授权版本传播讲法.md` |
| Identity 图 | `docs/04-身份Identity`、`docs/07-宣讲/05-Identity与ProfileLink讲法.md` |
| IDP 图 | `docs/02-认证AuthN/07-*`、`docs/07-宣讲/06-IDP与第三方登录讲法.md` |
| 接入图 | `docs/05-接入与契约`、`docs/07-宣讲/09-REST-gRPC-SDK接入讲法.md` |
| Suggest 图 | `docs/08-Suggest`、`api/rest/suggest.v2.yaml`、`internal/apiserver/transport/rest/suggest`、`internal/apiserver/application/suggest`、`internal/apiserver/domain/suggest`、`internal/apiserver/infra/suggest`、`internal/apiserver/infra/mysql/suggest` |
| 工程护栏图 | `docs/06-架构护栏`、`docs/07-宣讲/10-工程质量与架构护栏讲法.md` |

不要把已归档的专题分析作为当前证据源。

---

## 本文总结

`12-架构图素材索引.md` 的作用是：

```text
帮你把 IAM 项目的关键设计转成可讲、可画、可追问的图。
```

最重要的不是图多，而是图服务讲法。

IAM 对外宣讲建议永远围绕这条主线组织图：

```text
项目定位
  -> 系统架构
  -> AuthN
  -> AuthZ
  -> Identity
  -> IDP
  -> 接入
  -> Suggest
  -> 工程护栏
```

面试优先准备 5 张主线图：

```text
IAM 总定位图；
分层架构主图；
AuthN 登录主链路图；
AuthZ 授权模型 / 写入链路图；
User / Profile / ProfileLink 图。
```

如果还有余力，补第 6 张：

```text
Suggest 查询链路图。
```

这 6 张图讲清楚，IAM 项目的核心能力、复杂度和工程价值就基本能讲出来。

如果只记住一句话：

```text
架构图不是为了展示复杂度，而是为了用最少的视觉元素讲清边界、链路、取舍和证据；Suggest 图尤其要讲清它是 Profile 联想搜索辅助读模型，而不是 IAM 核心身份域、完整搜索服务或 AuthZ 权限中心。
```
