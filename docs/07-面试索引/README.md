...
| 维度 | 可公开口径 | 证据 |
| --- | --- | --- |
| 项目规模 | 五个业务模块；REST 与 gRPC 双协议；当前 proto 共 12 个 gRPC service；仓库 Schema 目标和最近一次生产验收均为 000028，但仓库门禁、迁移执行和运行健康仍需分层表述 | [模块划分](../00-概览/02-模块划分与协作关系.md)、[接口契约治理](../04-接口与SDK/01-REST-gRPC与契约治理.md)、[MySQL 事务与迁移](../03-基础设施/01-MySQL事务与迁移.md) |
