...
这里我没有把它设计成“查用户表后发 JWT”。登录请求会先进入 LoginApplicationService。SignIn 会通过 MethodSelector 和 SignInAdapterCatalog 选择登录方式，比如 password、phone_otp、wechat、wecom。不同登录方式的 payload 会被 adapter 转成领域 proof，然后交给 domain Authenticator。认证成功后得到 Principal，里面包含 UserID、AccountID、TenantID、AMR 和 claims。

接下来 TokenIssuer 不会直接只发一个 JWT，而是先创建 Session。Session 是在线登录态锚点。然后签发短期 Access Token，并把 SessionID 写到 JWT claims 里。最后生成随机 Refresh Token，保存到服务端 store，Refresh Token 也绑定 Session。
...
sequenceDiagram
    ...
    SignIn->>Issuer: IssueToken
    Issuer->>Session: Create
    Issuer->>Issuer: Issue AccessToken
    Issuer->>Store: Save RefreshToken
```
...
AuthN 负责登录态。不同登录方式通过 adapter 转成 proof，Authenticator 认证出 Principal，TokenIssuer 创建 Session，签发 Access Token，保存 Refresh Token。JWKS 支持本地验签，在线 Verify 检查 revoked marker、Session 和 User/Account 状态。
