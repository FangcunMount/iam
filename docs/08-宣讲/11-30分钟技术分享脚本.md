...
本文是 `08-宣讲` 模块的整合型讲稿，用于你对外讲解 IAM 项目，尤其适合：
...
本文不是事实层文档，也不是源码说明书。  
它的目标是把前面几篇宣讲材料组织成一场 **30 分钟可讲、可控、可追问** 的技术分享。
...
3. 关键链路如何设计：Session/Token、AuthZ Check、ProfileLink、Outbox；
4. 项目如何通过 REST/gRPC/SDK 和架构护栏支撑长期演进。
```
...
| 9:00 - 14:00 | AuthN 认证体系 | 讲登录态、Session、Token、JWKS/Verify |
...
一句话说，IAM 解决的是：在多入口、多服务、多身份源的业务系统里，谁能登录、登录态是否有效、能访问什么资源、和哪些业务档案有关，以及其他业务系统如何稳定接入这些能力。
```
...
```text
这个项目的背景是：业务系统不是一个单体页面，而会有 Web、App、微信小程序、管理后台、内部服务、worker 等多种入口。
...
第四，第三方身份源容易污染登录逻辑。微信/企微登录会涉及 AppSecret、access_token、code2Session，如果直接写进 LoginService，登录态和外部平台配置会混在一起。
...
```text
IAM:
AuthN / AuthZ / Identity / IDP / REST-gRPC-SDK
```
...
纵向上，它采用分层和六边形架构。外部请求通过 REST、gRPC 或 SDK 进入系统，Transport 层只做协议适配，比如路由、参数绑定、错误映射和 middleware。真正的用例编排在 Application 层，比如登录、Verify、AuthZ Check、授权写入、ProfileLink 操作。Domain 层承载业务规则，比如 Session 是否 active、RoleBinding 是否成立、ProfileLink 是否重复。Infra 层适配 MySQL、Redis、Casbin、JWT、Outbox 和微信 API。
...
这里我没有把它设计成“查用户表后发 JWT”。登录请求会先进入 LoginApplicationService。SignIn 会通过 MethodRegistry 和 LoginMethod 选择登录方式，比如 password、phone_otp、wechat、wecom，再由 ProofFactory 把 payload 转成领域 proof，然后交给 domain Authenticator。认证成功后得到 Principal，里面包含 UserID、AccountID、TenantID、AMR 和 claims。

接下来 SessionTokenIssuer 不会直接只发一个 JWT，而是先创建 Session。Session 是在线登录态锚点。然后 SessionTokenPairIssuer 签发短期 Access Token，并把 SessionID 写到 JWT claims 里。最后生成随机 Refresh Token，保存到服务端 store，Refresh Token 也绑定 Session。

这样设计是为了解决长期 JWT 撤销困难的问题。Access Token 是短期访问凭证，适合请求携带和本地验签；Refresh Token 是服务端可控的续期凭证；Session 是登录态锚点，可以被 revoke。
...
所以 AuthN 的核心亮点是：多种登录方式最终统一到 Principal、Session、Access Token、Refresh Token、JWKS 和在线 Verify 这一套模型。
```
...
sequenceDiagram
    ...
    participant SessionIssuer
    participant Session
    participant PairIssuer
    participant Store

    Client->>SignIn: auth_method + payload
    SignIn->>Authenticator: proof
    Authenticator-->>SignIn: Principal
    SignIn->>SessionIssuer: IssueToken
    SessionIssuer->>Session: Create
    SessionIssuer->>PairIssuer: IssueTokenPair
    PairIssuer->>Store: Save RefreshToken
```

### 重点句

```text
JWT 证明 token 是 IAM 签的；Session 证明这次登录态仍然在线有效；Refresh Token 证明客户端有资格续期。
```
...
这里也不是 user.role 字段。我把授权模型拆成 Role、Resource、Permission、RoleBinding。Permission 表示某个 role 能对某个 resource 执行某个 action，并限制在某个 scope；RoleBinding 表示某个 subject 在某个 tenant 下持有某个 role。

判定链路是：业务服务发起 Check，传 subject、tenant、resource、action、scope。Application 层构造成 AuthorizationRequest，然后调用 DecisionEngine。当前 DecisionEngine 由 CasbinAdapter 实现，最终执行 Enforce，返回 allowed 或 denied。

这里要强调，Casbin 不是领域模型。领域语言是 Role、Resource、Permission、RoleBinding、Scope；Casbin 的 p/g facts 只是 infra 层 runtime 映射。
...
## 10. Identity 与 ProfileLink：19:00 - 22:00
...
注意，ProfileLink 是身份关系 guard，不是 AuthZ 权限。如果未来要把 Profile 操作纳入统一资源授权，应该通过 AuthZ Resource/Action/Scope 扩展，而不是把 ProfileLink 变成权限表。
```
...
以微信小程序登录为例，客户端提交 app_id 和 js_code。AuthN 的 wechat adapter 会向 IDP 查询 WechatApp，检查应用是否 enabled，再通过 SecretVault 解密 AppSecret，然后进行 code2Session。code2Session 成功只说明这个 code 对应某个微信 openid/unionid，不等于 IAM 登录成功。后续还要检查这个外部身份是否绑定 IAM Account/User，最后才由 AuthN 创建 Principal、Session 和 Token。

这里还有一个容易混淆的点：微信 access_token 不是 IAM access token。微信 access_token 是调用微信平台 API 的凭证，由 IDP 管；IAM access token 是访问 IAM 或业务系统的 JWT，由 AuthN 管。
```

### 重点句

```text
IDP 证明外部身份源如何接入，AuthN 决定外部身份能否登录 IAM。
```
...
## 12. REST / gRPC / SDK 接入：24:00 - 26:00
...
gRPC 面向可信服务间调用，事实源是 proto。比如 VerifyToken、AuthZ Check、AuthorizationSnapshot、Identity 系统侧查询、ProfileLink Query/Command、IDP 内部能力，都适合 gRPC。gRPC 还能结合 mTLS、service token、ACL 和 audit。

SDK 是 Go 服务接入 IAM 的产品化封装。业务服务理论上可以直接调 REST/gRPC，但每个服务自己写连接、metadata、JWKS、Verify、ServiceAuth、AuthZ Check 和错误处理会重复且容易出错。SDK 通过 sdk.NewClient 初始化 Auth、Authz、Identity、ProfileLink、IDP 子客户端；用户登录用 auth/loginv2，认证接入还包括 JWKSManager、TokenVerifier、ServiceAuthHelper。
...
第一是架构测试。比如 domain 不能依赖 infra/database，application 不能依赖 transport/infra，REST router 不能依赖 container 或 viper，assembler 不能退回 interface{} 装配。AuthZ 还有专项护栏，防止 Casbin p/g/r facts 进入 domain，保证领域语言仍然是 Role、Resource、Permission、RoleBinding。

第二是契约测试。REST 侧有 router matrix 和 OpenAPI 覆盖检查，确认关键路由已注册，并且 public routes 被 OpenAPI 覆盖。gRPC 侧有 proto contract test，扫描 proto 里的 service，并检查 transport/grpc/service 里是否有对应 Register<Service>Server 调用。
...
第一，AuthN 负责认证态，把多种登录方式统一到 Principal、Session、Access Token、Refresh Token、JWKS 和 Verify。
...
架构上采用分层与六边形架构，Transport 负责 REST/gRPC 协议适配，Application 编排用例和事务，Domain 承载业务规则，Infra 适配 MySQL、Redis、Casbin、JWT、Outbox 和微信 API，Container 作为组合根，Process 管生命周期。

AuthN 负责登录态。不同登录方式通过 LoginMethod 和 ProofFactory 转成 proof，Authenticator 认证出 Principal，SessionTokenIssuer 创建 Session，SessionTokenPairIssuer 签发 Access Token、保存 Refresh Token。JWKS 支持本地验签，在线 Verify 检查 revoked marker、Session 和 User/Account 状态。

AuthZ 负责访问权。模型是 Role、Resource、Permission、RoleBinding，Check 输入 subject、tenant、resource、action、scope，最后通过 CasbinAdapter 判定。写入不是 CRUD，而是 UoW 写 facts、递增 PolicyVersion、stage Outbox，并 reload runtime policy。
...
```text
为什么不用长期 JWT？
Refresh Token 为什么不做 JWT？
JWKS 和在线 Verify 的区别？
用户 block 后旧 token 怎么失效？
Service Token 和用户 Token 区别？
```
...
| 分享内容 | 可回链文档 |
| --- | --- |
| 项目定位 | `08-宣讲/00-项目一句话定位.md` |
| 业务背景 | `08-宣讲/01-业务背景与问题.md` |
| 系统架构 | `08-宣讲/02-系统架构讲法.md` |
| AuthN | `08-宣讲/03-AuthN认证体系讲法.md` |
| AuthZ | `08-宣讲/04-AuthZ授权体系讲法.md` |
| Identity/ProfileLink | `08-宣讲/05-Identity与ProfileLink讲法.md` |
| IDP | `08-宣讲/06-IDP与第三方登录讲法.md` |
| JWKS/Token 安全 | `08-宣讲/07-JWKS与Token安全讲法.md` |
| Outbox | `08-宣讲/08-Outbox与授权版本传播讲法.md` |
| REST/gRPC/SDK | `08-宣讲/09-REST-gRPC-SDK接入讲法.md` |
| 工程质量 | `08-宣讲/10-工程质量与架构护栏讲法.md` |
| 专题分析 | `07-专题分析/*` |
| 事实层 | `00-概览` 到 `06-架构护栏` |
