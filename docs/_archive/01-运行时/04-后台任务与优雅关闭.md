...
本文回答：IAM 在 HTTP/gRPC 请求处理之外，还运行了哪些后台任务；这些任务如何从业务模块暴露给 process；Outbox Relay 如何从数据库 outbox 中异步投递事件；服务关闭时如何停止后台任务、清理模块资源、关闭数据库、HTTP 与 gRPC。

读完本文，你应该能回答：

- IAM 当前有哪些后台任务；
- 后台任务为什么不在 `main` 或 handler 中启动；
- `Container.BuildRuntimeDeps()` 的作用是什么；
- AuthN/JWKS key rotation scheduler 如何启动和停止；
- Outbox relay 的 claim/publish/mark 流程是什么；
- EventBus 不可用时，为什么 relay 不 claim outbox 事件；
- Suggest 模块的 cron/refresher 如何进入关闭流程；
- `preparedAPIServer.Run()` 如何同时运行 HTTP 和 gRPC；
- graceful shutdown 的顺序是什么；
- shutdown 中每一步失败会如何处理。

本文只讨论运行时生命周期，不展开 AuthZ UoW、JWKS key model、Suggest 索引刷新算法等业务细节。
...
| EventBus 不可用怎么办 | outbox store 可保留，relay 不启动；事件保持 pending/failed。 | [../../internal/apiserver/container/eventing_test.go](../../internal/apiserver/container/eventing_test.go) |
...
`startRuntimeTasks` 当前处理两类“主动启动”的后台任务：
...
这意味着业务事务仍可以把事件 stage 到 `domain_event_outbox`，但不会启动 relay 投递。事件会保留在数据库中，等待 EventBus 可用后再投递。
...
## 9. preparedAPIServer.Run 与 RunGroup
...
### 11.1 为什么先 lifecycle hooks？

Outbox relay 和 key rotation scheduler 属于后台任务。  
它们可能使用数据库、事件总线、key repository 等资源。

因此应先停止后台任务，再关闭数据库和 server。

### 11.2 为什么 DB 在 HTTP/gRPC 之前关闭？
...
| EventBus 不可用 | relay 不启动或 DispatchDue degraded return nil | outbox rows 保留 pending/failed |
...
这篇文档是后续理解 AuthN/JWKS、AuthZ/Outbox、Suggest 刷新机制的运行时基础。
