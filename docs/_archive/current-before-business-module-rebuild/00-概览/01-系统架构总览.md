...
它围绕“用户是谁、如何证明用户身份、用户能访问什么资源”三个核心问题，统一提供：

```text
- 身份：表达“系统内部这个人是谁，以及他关联了哪些业务档案”。
- 认证：识别“你是谁”，并签发可验证的访问凭证。
- 授权：表达“你能访问什么资源、执行什么操作”，并维护可传播的授权事实。
- 外部身份源：对接微信、企业微信等第三方身份提供方，将外部身份映射为系统内账号或用户。
- 接入产品化：通过 REST、gRPC、SDK 等多种接入方式，支持业务系统接入。
- 辅助读模型：内置 Suggest，为 operating 后台提供 Profile 联想搜索能力，用于用户档案的自动补全和联想搜索。
```

一句话：

> IAM 以 User 为稳定身份锚点，以 AuthN 解决登录与 Token，以 AuthZ 解决资源访问控制，以 Identity 解决 User/Profile/ProfileLink 身份事实，以 IDP 适配外部身份源；
> 通过 REST/gRPC/SDK 向业务系统提供身份与访问管理能力;
> 同时在 iam-apiserver 内暂存 Suggest 辅助读模型，用于 operating 后台的 Profile 联想搜索。
...
```text
1. IAM 提供核心的身份、认证、授权模型与接入能力；
2. 业务系统使用这些能力完成自己的业务流程：
   - 如果要表达业务身份关系，应进入 Identity Profile / ProfileLink；
   - 如果要认证用户身份，应进入 AuthN LoginIdentity / Credential / Session / Token；
   - 如果要表达业务资源与角色权限关系，并进行授权与权限检查，应进入 AuthZ Role / Permission / Resource / Action / Scope / Check。
```
...
### 7.1 [Identity：身份事实模块](docs/04-身份Identity/README.md)
...
### 7.2 [AuthN：认证模块](docs/02-认证AuthN/README.md)
...
### 7.3 [AuthZ：授权模块](docs/03-授权AuthZ/README.md)
...
### 7.4 [IDP：外部身份源基础设施](docs/05-接入与契约/README.md)
...
### 7.5 [Suggest：Profile 联想搜索读模型](docs/08-Suggest/README.md)
