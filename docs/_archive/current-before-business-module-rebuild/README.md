...
| `01-运行时` | 解释服务如何启动、装配 REST/gRPC、运行后台任务、优雅关闭和健康检查 |
| `02-认证AuthN` | 讲解 LoginIdentity、Credential、Challenge、Principal、Session、Token、JWKS，以及 Onboarding、Linking、Login、Token、Session 链路 |
| `03-授权AuthZ` | 讲解 Subject、Role、Resource、Permission、RoleBinding、授权写入、授权版本传播、权限检查和 Casbin Runtime |
| `04-身份Identity` | 讲解 User、Profile、ProfileLink，以及 Identity 与 AuthN、AuthZ、Suggest 的边界 |
| `05-接入与契约` | 说明 REST、gRPC、SDK 三类接入方式、业务系统接入链路和契约防漂移机制 |
| `06-架构护栏` | 说明架构测试、契约测试、SDK compile test、docs-hygiene 如何防漂移 |
| `07-宣讲` | 技术分享、对外讲解、架构图素材和追问证据链 |
| `08-Suggest` | 说明 Suggest Profile 联想搜索读模型、查询链路、权限范围、索引刷新、安全与运维 |
| `_archive` | 历史文档归档，不作为当前事实源 |
...
│   ├── 02-Identity与AuthN 边界--身份认证-Principal-User.md
│   ├── 03-Identity与AuthZ 边界--Subject-Resource-Permission.md
...
| 核心概念如何区分 | [00-概览/02-核心概念术语.md](00-概览/02-核心概念术语.md) |
| 代码组织和事实源优先级是什么 | [00-概览/03-阅读路径-代码组织与事实来源.md](00-概览/03-阅读路径-代码组织与事实来源.md) |
| 服务如何启动、装配和关闭 | [01-运行时/README.md](01-运行时/README.md)、[01-运行时/01-服务入口与生命周期装配.md](01-运行时/01-服务入口与生命周期装配.md)、[01-运行时/04-后台任务与优雅关闭.md](01-运行时/04-后台任务与优雅关闭.md) |
| REST/gRPC 如何装配 | [01-运行时/02-Transport装配--REST路由与gRPC服务注册.md](01-运行时/02-Transport装配--REST路由与gRPC服务注册.md)、[05-接入与契约/01-REST API契约-前端与管理端接入.md](05-接入与契约/01-REST API契约-前端与管理端接入.md)、[05-接入与契约/02-gRPC API契约-服务间调用与内部集成.md](05-接入与契约/02-gRPC API契约-服务间调用与内部集成.md) |
| 配置、运行模式、降级启动如何影响服务 | [01-运行时/03-配置与运行模式.md](01-运行时/03-配置与运行模式.md)、[01-运行时/05-降级启动与健康检查.md](01-运行时/05-降级启动与健康检查.md) |
| 登录身份、凭证、挑战如何建模 | [02-认证AuthN/README.md](02-认证AuthN/README.md)、[02-认证AuthN/00-AuthN模型总览-User-LoginIdentity-Credential-Challenge.md](02-认证AuthN/00-AuthN模型总览-User-LoginIdentity-Credential-Challenge.md) |
...
| 登录身份如何绑定、解绑和治理 | [02-认证AuthN/02-Linking链路-登录身份绑定解绑与安全边界.md](02-认证AuthN/02-Linking链路-登录身份绑定解绑与安全边界.md) |
...
| 授权模型如何组织 | [03-授权AuthZ/README.md](03-授权AuthZ/README.md)、[03-授权AuthZ/00-AuthZ模型总览-Subject-Role-Resource-Permission-RoleBinding.md](03-授权AuthZ/00-AuthZ模型总览-Subject-Role-Resource-Permission-RoleBinding.md) |
...
| User、Profile、ProfileLink 如何建模 | [04-身份Identity/README.md](04-身份Identity/README.md)、[04-身份Identity/00-Identity模型总览-User-Profile-ProfileLink.md](04-身份Identity/00-Identity模型总览-User-Profile-ProfileLink.md)、[04-身份Identity/01-ProfileLink链路-User与Profile关系协作.md](04-身份Identity/01-ProfileLink链路-User与Profile关系协作.md) |
| Identity 与 AuthN / AuthZ 边界是什么 | [04-身份Identity/02-Identity与AuthN 边界--身份认证-Principal-User.md](04-身份Identity/02-Identity与AuthN%20边界--身份认证-Principal-User.md)、[04-身份Identity/03-Identity与AuthZ 边界--Subject-Resource-Permission.md](04-身份Identity/03-Identity与AuthZ%20边界--Subject-Resource-Permission.md) |
...
| Suggest 权限、索引、刷新、安全如何设计 | [08-Suggest/02-权限范围-OperatingPrincipal与ProfileAccessScope.md](08-Suggest/02-权限范围-OperatingPrincipal与ProfileAccessScope.md)、[08-Suggest/03-索引模型-ProfileSearchTerm-Trie-Hash-Runtime.md](08-Suggest/03-索引模型-ProfileSearchTerm-Trie-Hash-Runtime.md)、[08-Suggest/04-刷新链路-Loader-Refresher-FullDelta-Snapshot.md](08-Suggest/04-刷新链路-Loader-Refresher-FullDelta-Snapshot.md)、[08-Suggest/05-安全与运维-手机号搜索-限流-指标-降级.md](08-Suggest/05-安全与运维-手机号搜索-限流-指标-降级.md) |
...
| qs-server 如何接入 IAM | [05-接入与契约/04-业务系统接入链路-qs-server接入 IAM 详解.md](05-接入与契约/04-业务系统接入链路-qs-server接入%20IAM%20详解.md) |
...
| 如何对外讲解、面试准备、技术分享 | [07-宣讲/README.md](07-宣讲/README.md)、[07-宣讲/11-30分钟技术分享脚本.md](07-宣讲/11-30分钟技术分享脚本.md)、[07-宣讲/13-追问回链证据索引.md](07-宣讲/13-追问回链证据索引.md) |
...
```text
按模型 -> ProfileLink 链路 -> AuthN 边界 -> AuthZ 边界 -> 分层事实源的顺序理解 Identity。
```
...
```text
理解 Suggest 为什么是 Profile 联想搜索读模型，如何在不拆独立服务的情况下完成 autocomplete、权限过滤、索引刷新、手机号安全、限流、指标和降级。
```
...
9. 术语统一：LoginIdentity / ProfileLink / RoleBinding / Assignment / Session / Outbox / SDK / IDP / Suggest / ProfileAccessScope；
