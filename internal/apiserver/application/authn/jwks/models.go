...
func (k *ManagedKey) IsActive() bool {
	return k.Status == KeyActive
}

func (k *ManagedKey) IsGrace() bool {
	return k.Status == KeyGrace
}

func (k *ManagedKey) IsRetired() bool {
	return k.Status == KeyRetired
}

func (k *ManagedKey) CanSign() bool {
	return k.IsActive() && !k.IsExpired(time.Now())
}

func (k *ManagedKey) CanVerify() bool {
	return (k.IsActive() || k.IsGrace()) && !k.IsExpired(time.Now())
}

func (k *ManagedKey) ShouldPublish() bool {
	return (k.IsActive() || k.IsGrace()) && !k.IsExpired(time.Now())
}

func (k *ManagedKey) IsExpired(now time.Time) bool {
	return k.NotAfter != nil && now.After(*k.NotAfter)
}

func (k *ManagedKey) IsNotYetValid(now time.Time) bool {
	return k.NotBefore != nil && now.Before(*k.NotBefore)
}

func (k *ManagedKey) IsValidAt(t time.Time) bool {
	return !k.IsExpired(t) && !k.IsNotYetValid(t)
}

func (k *ManagedKey) EnterGrace() error {
	if !k.IsActive() {
...
func (k *ManagedKey) Retire() error {
	if !k.IsGrace() {
...
func (k *ManagedKey) ForceRetire() {
	k.Status = KeyRetired
	k.UpdatedAt = time.Now()
}

func (k *ManagedKey) Validate() error {
	if k.Kid == "" {
		return errors.WithCode(code.ErrInvalidKid, "kid cannot be empty")
	}
