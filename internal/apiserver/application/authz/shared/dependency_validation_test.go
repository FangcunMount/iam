...
func TestValidateResourceDependenciesRejectsInvalidCandidate(t *testing.T) {
	...
	err := shared.ValidateResourceDependencies(candidate, []*permissiongrant.Grant{&grant})
...
func TestValidateResourceDependenciesAcceptsCompatibleCandidate(t *testing.T) {
	...
	require.NoError(t, shared.ValidateResourceDependencies(candidate, []*permissiongrant.Grant{&grant}))
}
...
func TestEnsureRoleUnusedRejectsEveryActiveDependencyType(t *testing.T) {
	...
	binding, err := rolebinding.NewBinding(rolebinding.SubjectTypeUser, meta.FromUint64(22), roleID, "tenant-a", rolebinding.WithGrantedBy("operator"))
	...
	tests := []struct {
		...
		{name: "assignment", bindings: []*rolebinding.Binding{&binding}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := shared.EnsureRoleUnused(roleID, tt.bindings, tt.grants, tt.inheritances)
			require.Error(t, err)
...
func TestAffectedResourceTenantIDsIncludesAllActiveGrantTenants(t *testing.T) {
	...
	require.Equal(t, []string{"tenant-a", "tenant-b", "tenant-operator"}, shared.AffectedResourceTenantIDs(
		"tenant-operator",
