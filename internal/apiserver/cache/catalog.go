...
var catalog = []FamilyDescriptor{
	...
	},
	{
		...
		Backend:         BackendKindRedis,
		RedisType:       RedisDataTypeZSet,
		Codec:           ValueCodecKindLeaseToken,
		Role:            DataRoleMarkerState,
		OwnerModule:     "authn",
		KeyPattern:      "otp:quota:{scene}:{phoneE164}:{dimension}",
		TTLSource:       "滑动窗口长度",
		SelectionReason: "同号码同场景的小时/天发送限量需要按最近窗口内的发送记录计数。",
		Policy: FamilyPolicy{
			TTLSource:                      "滑动窗口长度",
			WriteMode:                      "Lua ZREMRANGEBYSCORE + ZCARD + ZADD + PEXPIRE",
			InvalidationMode:               "窗口外成员清理、TTL 到期或失败补偿 ZREM",
			HasInternalRefreshCoordination: false,
		},
		Capabilities: inspectOnly,
	},
