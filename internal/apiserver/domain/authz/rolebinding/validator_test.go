...
func TestValidateGrantAndRevokeCommands_Invalids(t *testing.T) {
	...
	// empty grant command
	err := v.ValidateGrantParameters("", "", 0, "", "")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrInvalidArgument))

	// empty revoke command
	err = v.ValidateRevokeParameters("", "", 0, "")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrInvalidArgument))

	err = v.ValidateGrantParameters(binding.SubjectTypeGroup, "100", 1, "t1", "1")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrInvalidArgument))

	err = v.ValidateRevokeParameters(binding.SubjectTypeService, "svc-1", 1, "t1")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrInvalidArgument))
...
func TestFindBindingBySubjectAndRole_FoundAndNotFound(t *testing.T) {
	a1 := &binding.Binding{SubjectType: binding.SubjectTypeUser, SubjectID: "u1", RoleID: 11, TenantID: "t"}
	...
	asg, err := v.FindBindingBySubjectAndRole(context.Background(), binding.SubjectTypeUser, "u1", 11, "t")
	...
	asg2, err2 := v2.FindBindingBySubjectAndRole(context.Background(), binding.SubjectTypeUser, "u1", 99, "t")
...
func TestCheckSubjectExists_OnlySupportsExistingUsers(t *testing.T) {
	...
	err := v.CheckSubjectExists(context.Background(), binding.SubjectTypeGroup, "group-1", "t1")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrInvalidArgument))

	err = v.CheckSubjectExists(context.Background(), binding.SubjectTypeUser, "999", "t1")
	require.Error(t, err)
	assert.True(t, perrors.IsCode(err, code.ErrUserNotFound))

	err = v.CheckSubjectExists(context.Background(), binding.SubjectTypeUser, "123", "t1")
	require.NoError(t, err)
}
