...
func TestScopePolicy_AllProfile(t *testing.T) {
	p := ScopePolicy{}
	term := NewProfileSearchTerm(1, "a", nil, 1, 999, 999, []int64{999})
...
func TestScopePolicy_ProfileIDs(t *testing.T) {
	p := ScopePolicy{}
	term := NewProfileSearchTerm(5, "a", nil, 1, 1, 0, nil)
...
func TestScopePolicy_TenantOrgOperator(t *testing.T) {
	p := ScopePolicy{}
	term := NewProfileSearchTerm(1, "a", nil, 1, 10, 20, []int64{100, 200})
	if !p.Allows(ProfileAccessScope{TenantIDs: []int64{10}}, term) {
...
func TestScopePolicy_EmptyScopeDenies(t *testing.T) {
	p := ScopePolicy{}
	term := NewProfileSearchTerm(1, "a", nil, 1, 1, 1, []int64{1})
...
func TestScopePolicy_OrgIDZeroIgnored(t *testing.T) {
	p := ScopePolicy{}
	term := NewProfileSearchTerm(1, "a", nil, 1, 0, 0, nil)
...
func TestScopePolicy_CompiledLargeProfileIDs(t *testing.T) {
	...
	term := NewProfileSearchTerm(250, "a", nil, 1, 1, 1, nil)
	if !p.AllowsCompiled(scope, term) {
		t.Fatal("expected hit in compiled profile set")
	}
	term2 := NewProfileSearchTerm(99999, "b", nil, 1, 1, 1, nil)
