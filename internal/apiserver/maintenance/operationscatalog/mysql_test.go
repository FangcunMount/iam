...
func mysqlDatabase(t *testing.T) *gorm.DB {
	t.Helper()
	raw := os.Getenv("OPERATIONS_CATALOG_MYSQL_DSN")
	if raw == "" {
		if os.Getenv("OPERATIONS_CATALOG_REQUIRE_MYSQL") == "true" {
			t.Fatal("OPERATIONS_CATALOG_MYSQL_DSN required")
		}
		return nil
	}
	...
	name := fmt.Sprintf("iam_operations_test_%d", time.Now().UnixNano())
	if !strings.HasPrefix(name, "iam_operations_test_") {
