...
func parseScope(c *gin.Context, scopeType, scopeValue string) (scope.Scope, bool) {
	scopeValueObject, err := scope.Normalize(scopeType, scopeValue)
	if err != nil {
		handleError(c, err)
		return scope.Scope{}, false
	}
	return scopeValueObject, true
}
