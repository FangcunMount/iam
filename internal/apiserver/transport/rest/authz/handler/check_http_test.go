...
func TestCheckHandlerHTTPBranches(t *testing.T) {
	t.Run("nil casbin returns internal server error", func(t *testing.T) {
		...
		recorder, _ := performAuthzRequest(http.MethodPost, "/check", `{"object":"scale:form:*","action":"read"}`, handler.Check, withTenantUser("tenant-a", "user-1"))
	...
	t.Run("bind error does not call casbin", func(t *testing.T) {
		...
		recorder, _ := performAuthzRequest(http.MethodPost, "/check", `{}`, handler.Check, withTenantUser("tenant-a", "user-1"))
	...
	t.Run("missing tenant returns token invalid", func(t *testing.T) {
		...
		recorder, _ := performAuthzRequest(http.MethodPost, "/check", `{"object":"scale:form:*","action":"read"}`, handler.Check, withUser("user-1"))
	...
	t.Run("explicit subject is used when present", func(t *testing.T) {
		...
		require.Equal(t, "user:user-2", casbin.enforceCalls[0].sub)
	...
	t.Run("current user fallback is used when subject is omitted", func(t *testing.T) {
		...
		requireAuthzCode(t, recorder, http.StatusOK, 200)
		require.Len(t, casbin.enforceCalls, 1)
		require.Equal(t, "user:user-1", casbin.enforceCalls[0].sub)
	})
