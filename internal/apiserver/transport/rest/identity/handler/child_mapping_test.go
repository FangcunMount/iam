...
func TestChildResultToResponse(t *testing.T) {
	result := &appchild.ChildResult{
		ID:       "child-1",
		Name:     "Kid",
	...
	empty := childResultToResponse(nil)
	if empty != (responsedto.ChildResponse{}) {
		t.Fatalf("unexpected nil child response: %#v", empty)
	}
}

func TestChildMappingParsers(t *testing.T) {
	height := 121
...
func TestSliceChildren(t *testing.T) {
	items := []responsedto.ChildResponse{
		{ID: "1"},
		{ID: "2"},
		{ID: "3"},
	}

	sliced := sliceChildren(items, 1, 1)
	if len(sliced) != 1 || sliced[0].ID != "2" {
		t.Fatalf("unexpected slice: %#v", sliced)
	}

	if out := sliceChildren(items, 10, 1); len(out) != 0 {
