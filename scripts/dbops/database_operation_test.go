...
case "$*" in
  *'SELECT 1;'*) echo '1' ;;
	  ...
	  *'MAX(version)'*) printf '%b\n' "${IAM_FAKE_MIGRATION_STATE:-37\t0\t1}" ;;
...
`)
	...
	for name, overrides := range map[string]map[string]string{
		...
		"dirty scope schema": {
			"IAM_FAKE_MIGRATION_STATE": "37\t1\t1",
		},
