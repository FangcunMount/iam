...
| Fresh bootstrap 33/35 | Original insert-only Stager before standard migration 38 | Retained one-time legacy bootstrap; actual drain and sequential messaging-process handoff proven in isolation, complete-service shutdown is verified during the reviewed production cutover |
...
The isolated proof delivers both original notifications and a later standard message through NSQ using sequential child processes. It also checks a clean restart and a drained return to legacy mode. Those children use actual messaging composition but are not the full API service. The user has narrowed first-release rehearsal scope: reuse this evidence, then verify actual process exit, current subscriber topology, writer freeze and business results during the reviewed production cutover; a separate expanded multi-instance signal matrix is deferred.
...
The first-release gate reuses the completed transaction, recovery, wire, shutdown, sequential child-process and isolated IAM-to-QS revoke/restore proofs. Remaining pre-release work is reviewed SDK/version availability and green IAM CI, plus the concrete freeze/drain/cutover/rollback plan. At production cutover, verify actual old-process exit and every current required subscriber, then observe a bounded initial window with real business checks. Extended capacity, multi-fault and undeployed topology matrices move to M6; this deferral does not assert they passed. qs-ai execution/recovery files remain outside this change.

## Deployment configuration and schema checks

The deployment variable `IAM_RELIABLE_MESSAGING_ENABLED` is copied into the generated package as `IAM_APISERVER_EVENTS_RELIABLE_MESSAGING_ENABLED`; the production YAML explicitly declares its key so Viper unmarshals the environment override. Default is false; only true/false are accepted. Keep the reviewed mode in this persistent deployment setting so later package generation does not silently reset a manual remote YAML edit. Do not change the live setting before freeze/drain and approval.

Production Compose allows 60s for process exit, covering the configured 5s readiness delay and default 20s SDK drain with room for other hooks. This is an outer limit, not evidence that all hooks completed. The old container may still have the previous stop configuration: give its first controlled stop an explicit 60s timeout and inspect the outcome. A shorter command-line timeout must not override this budget.

The database status script now requires all 20 runtime tables at clean schema 38, including rm_outbox, and preserves the optional retirement-audit exception. Real MySQL status accepts the complete schema and rejects a missing standard table. It does not replace message preflight or establish writer exclusion.

For the current application-start migration mechanism, the reviewed new image can first run in legacy mode under the write freeze to install migration 38. After its data checks and actual stop, run SDK preflight and enable SDK with the same image. That schema-aware image provides the legacy fallback; the old M2 image is not automatically compatible. The existing authorization-migrate command accepts only 31/32 and must not be used as a migration-38 CLI. Retain both tables and avoid schema down.
