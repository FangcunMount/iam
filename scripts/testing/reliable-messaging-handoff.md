...
The first-release gate now reuses the completed transaction, recovery, wire, shutdown and sequential child-process proofs. Remaining pre-release work is reviewed SDK/version availability and green IAM CI, one real IAM-to-QS revoke/restore decision flow through the actual subscriber entry points, and a concrete freeze/drain/cutover/rollback plan for user approval. At production cutover, verify actual old-process exit and every current required subscriber, then observe a bounded initial window with real business checks. Extended capacity, multi-fault and undeployed topology matrices move to M6; this deferral does not assert they passed. qs-ai execution/recovery files remain outside this change.
